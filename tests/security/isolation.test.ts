...
describe('isolation & tenancy security', () => {
  ...
  it('membership revoked mid-session takes effect immediately', async () => {
    ...
    const cashierReg = await t.request.post('/v1/auth/register').send({
      email: uniqueEmail(), password: 'Str0ng!Passw0rd', displayName: 'C', preferredLocale: 'ar',
    });
    ...
    await t.request
      .delete(`/v1/businesses/current/members/${cashierId}`)
      .set('Authorization', `Bearer ${a.token}`)
      .set('X-Business-Id', a.businessId);
  ...
  it('branch→warehouse cross-business FK rejected at DB level (§39)', async () => {
    ...
    const aBranch = (
      await ownerPool().query('SELECT id FROM branches WHERE business_id = $1 AND is_default', [a.businessId])
    ).rows[0]?.id as string;
    ...
    const bTenant = (
      await ownerPool().query('SELECT tenant_id FROM businesses WHERE id = $1', [b.businessId])
    ...
    await c.query(`SELECT set_config('app.tenant_id', $1, true), set_config('app.business_id', $2, true), set_config('app.bypass_rls', 'false', true)`, [
      bTenant, b.businessId,
    ]);
    const res = await c
      .query('INSERT INTO warehouses (business_id, id, branch_id, name) VALUES ($1, gen_random_uuid(), $2, $3)', [
        b.businessId, aBranch, 'cross',
      ])
  ...
  describe('owner safety (§28)', () => {
    it('removing the LAST owner fails; second owner allows removal of the first', async () => {
      ...
      const ownerRole = (
        await ownerPool().query(`SELECT id FROM business_roles WHERE business_id = $1 AND key = 'owner'`, [a.businessId])
      ).rows[0]?.id as string;
      ...
      await ownerPool().query('INSERT INTO membership_roles (business_id, user_id, role_id) VALUES ($1, $2, $3)', [
        a.businessId, ownerBId, ownerRole,
      ]);
  ...
  describe('system role immutability (§26)', () => {
    it('app role cannot INSERT/UPDATE/DELETE system roles even in bypass-free tx', async () => {
      ...
      const pool = new (await import('pg')).Pool({
        connectionString: 'postgresql://daftar_app:test_app_password_123@localhost:55432/daftar', max: 1,
      });
      const c = await pool.connect();
      const tenantId = (
        await ownerPool().query('SELECT tenant_id FROM businesses WHERE id = $1', [a.businessId])
      ).rows[0]?.tenant_id as string;
      await c.query('BEGIN');
      await c.query(`SELECT set_config('app.tenant_id', $1, true), set_config('app.business_id', $2, true), set_config('app.bypass_rls', 'false', true)`, [
        tenantId, a.businessId,
      ]);
      const ins = await c
      ...
      const del = await c
        .query(`DELETE FROM business_roles WHERE business_id = $1 AND key = 'owner'`, [a.businessId])
        .catch((e: { code: string }) => e);
    ...
    it('client cannot set is_system / owner key via API (mass assignment, §94–95)', async () => {
      ...
      const roles = await t.request
        .get('/v1/businesses/current/roles')
        .set('Authorization', `Bearer ${a.token}`)
        .set('X-Business-Id', a.businessId);
