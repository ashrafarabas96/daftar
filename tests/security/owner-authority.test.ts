...
import { Pool } from 'pg';
import { appDbUrl, createTestApp, ownerPool, platformDbUrl, resetData, uniqueEmail, type TestApp } from '../helpers/test-app';
...
describe('owner authority integrity (§2–9)', () => {
  ...
  async function fixture(): Promise<Fx> {
    ...
    const ownerUserId = must(
      (await ownerPool().query<{ user_id: string }>('SELECT user_id FROM memberships WHERE business_id = $1', [businessId])).rows[0],
    ).user_id;
    const otherUserId = (await t.request.get('/v1/auth/me').set('Authorization', `Bearer ${reg2.body.accessToken as string}`)).body.userId as string;
    ...
    const ownerRoleId = must(
      (await ownerPool().query<{ id: string }>(`SELECT id FROM business_roles WHERE business_id = $1 AND is_system AND key = 'owner'`, [businessId])).rows[0],
    ).id;
    const cashierRoleId = must(
      (await ownerPool().query<{ id: string }>(`SELECT id FROM business_roles WHERE business_id = $1 AND key = 'cashier'`, [businessId])).rows[0],
    ).id;
  ...
  it('daftar_app cannot DEMOTE an existing tenant_owner via raw SQL', async () => {
    ...
    await expect(
      withApp(fx, async (q) =>
        q(`UPDATE tenant_memberships SET role_key = 'tenant_member' WHERE tenant_id = $1 AND user_id = $2`, [fx.tenantId, fx.ownerUserId]),
      ),
  ...
  it('daftar_app cannot status-remove an existing tenant_owner via raw SQL', async () => {
    const fx = await fixture();
    await expect(
      withApp(fx, async (q) => q(`UPDATE tenant_memberships SET status = 'removed' WHERE tenant_id = $1 AND user_id = $2`, [fx.tenantId, fx.ownerUserId])),
    ).rejects.toThrow(/platform-managed/);
  });
  ...
  it('daftar_app cannot DELETE a tenant_owner row via raw SQL', async () => {
    const fx = await fixture();
    await expect(
      withApp(fx, async (q) => q(`DELETE FROM tenant_memberships WHERE tenant_id = $1 AND user_id = $2`, [fx.tenantId, fx.ownerUserId])),
    ).rejects.toThrow();
  });
  ...
  it('daftar_app cannot INSERT a system owner role grant into membership_roles (raw SQL escalation)', async () => {
    ...
    await expect(
      withApp(fx, async (q) =>
        q(`INSERT INTO membership_roles (business_id, user_id, role_id) VALUES ($1, $2, $3)`, [fx.businessId, fx.otherUserId, fx.ownerRoleId]),
      ),
  ...
  it('daftar_app cannot UPDATE an existing grant INTO the system owner role', async () => {
    ...
    await ownerPool().query(`INSERT INTO tenant_memberships (tenant_id, user_id, role_key) VALUES ($1, $2, 'tenant_member')`, [fx.tenantId, fx.otherUserId]);
    await ownerPool().query(`INSERT INTO memberships (business_id, user_id, tenant_id, status) VALUES ($1, $2, $3, 'active')`, [
      fx.businessId,
      fx.otherUserId,
      fx.tenantId,
    ]);
    await ownerPool().query(`INSERT INTO membership_roles (business_id, user_id, role_id) VALUES ($1, $2, $3)`, [
      fx.businessId,
      fx.otherUserId,
      fx.cashierRoleId,
    ]);
    await expect(
      withApp(fx, async (q) =>
        q(`UPDATE membership_roles SET role_id = $3 WHERE business_id = $1 AND user_id = $2 AND role_id = $4`, [
          fx.businessId,
          fx.otherUserId,
          fx.ownerRoleId,
          fx.cashierRoleId,
  ...
  it('ownership TRANSFER works: successor owner + demotion in ONE transaction', async () => {
    ...
    try {
      await c.query('BEGIN');
      await c.query(`SELECT set_config('app.bypass_rls', 'true', true)`);
      await c.query(`UPDATE tenant_memberships SET role_key = 'tenant_owner' WHERE tenant_id = $1 AND user_id = $2`, [fx.tenantId, fx.otherUserId]);
      await c.query(`UPDATE tenant_memberships SET role_key = 'tenant_member' WHERE tenant_id = $1 AND user_id = $2`, [fx.tenantId, fx.ownerUserId]);
      await c.query('COMMIT');
    } finally {
      c.release();
      await pool.end();
    }
    const { rows } = await ownerPool().query<{ role_key: string }>(`SELECT role_key FROM tenant_memberships WHERE tenant_id = $1 AND user_id = $2`, [
      fx.tenantId,
      fx.otherUserId,
    ]);
  ...
  it('concurrent demotion of two owners: exactly ONE succeeds (last-owner race)', async () => {
    const fx = await fixture();
    await ownerPool().query(`INSERT INTO tenant_memberships (tenant_id, user_id, role_key) VALUES ($1, $2, 'tenant_owner')`, [fx.tenantId, fx.otherUserId]);
    const attempt = async (userId: string): Promise<'ok' | 'fail'> => {
      ...
      try {
        await c.query('BEGIN');
        await c.query(`SELECT set_config('app.bypass_rls', 'true', true)`);
        await c.query(`UPDATE tenant_memberships SET role_key = 'tenant_member' WHERE tenant_id = $1 AND user_id = $2`, [fx.tenantId, userId]);
  ...
  it('golden kept: owner removed then re-added cashier holds cashier ONLY (§9)', async () => {
    ...
    const n = await withApp(fx, async (q) => {
      await q(`DELETE FROM membership_roles WHERE business_id = $1 AND user_id = $2`, [fx.businessId, fx.ownerUserId]);
      const r = (await q(`SELECT count(*)::text AS n FROM membership_roles WHERE business_id = $1 AND user_id = $2`, [fx.businessId, fx.ownerUserId])) as {
        rows: { n: string }[];
      };
