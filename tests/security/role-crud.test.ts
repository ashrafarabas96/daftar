...
describe('team role CRUD (§69–74)', () => {
  ...
  it('role.update renames and replaces the permission set (audited)', async () => {
    ...
    const res = await t.request
      ...
      .set(auth(o.token, o.businessId))
      .send({
        name: 'Senior Clerk',
        permissions: ['catalog.view', 'catalog.create'],
      });
    expect(res.status).toBe(200);
    expect(res.body.name).toBe('Senior Clerk');
    expect(res.body.permissions).toEqual(['catalog.view', 'catalog.create']);
    const { rows } = await ownerPool().query<{ action: string }>(`SELECT action FROM audit_events WHERE entity_id = $1 AND action = 'structure.role_updated'`, [
      roleId,
    ]);
  ...
  it('system roles are immutable: API 403 + DB trigger blocks raw UPDATE/DELETE', async () => {
    const o = await setup();
    const { rows } = await ownerPool().query<{ id: string }>(`SELECT id FROM business_roles WHERE business_id = $1 AND is_system AND key = 'owner'`, [
      o.businessId,
    ]);
  ...
  it('delegation ceiling applies to role UPDATE: a non-owner cannot widen a role', async () => {
    ...
    const res = await t.request
      .patch(`/v1/businesses/current/roles/${clerkId}`)
      .set(auth(leadToken, o.businessId))
      .send({
  ...
  it('delete-with-assigned: 409 ROLE_IN_USE without replacement; atomic reassignment with replacement', async () => {
    ...
    const { rows } = await ownerPool().query<{ role_id: string }>('SELECT role_id FROM membership_roles WHERE business_id = $1 AND user_id = $2', [
      o.businessId,
      memberId,
    ]);
    expect(rows.map((r) => r.role_id)).toEqual([viewerId]);
    const { rows: gone } = await ownerPool().query<{ id: string }>('SELECT id FROM business_roles WHERE id = $1', [clerkId]);
  ...
  it('member role assignment requires role.assign (member.manage alone is not enough)', async () => {
    ...
    const res = await t.request
      ...
      .set(auth(token, o.businessId))
      .send({ roleKeys: ['noassign'] });
    expect(res.status).toBe(403);
  });
});
