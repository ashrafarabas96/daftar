...
import {
  createTestApp, grantFeature, ownerPool, raiseLimit, resetData, uniqueEmail, type TestApp,
...
describe('feature gating + effective subscription state (§21–33)', () => {
  ...
  async function onboard(): Promise<{ token: string; businessId: string; userId: string }> {
    ...
    const on = await t.request.post('/v1/onboarding/complete').set('Idempotency-Key', `idem-${Date.now()}-${Math.floor(Math.random()*1e9)}`).set('Authorization', `Bearer ${token}`).send({
      businessName: 'G', countryCode: 'PS', baseCurrency: 'ILS',
  ...
  const createRole = (a: { token: string; businessId: string }) =>
    t.request.post('/v1/businesses/current/roles')
  ...
  it('feature FALSE + high limit → still denied (capability gates, §23/§25)', async () => {
    ...
    const res = await t.request.post('/v1/businesses/current/branches')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId)
      .send({ name: 'Second' });
  ...
  it('override FALSE after the TRUE window closes → denied (no overlapping windows, §25/§43)', async () => {
    ...
    await ownerPool().query(
      `UPDATE entitlement_overrides
       ...
       WHERE business_id = $1 AND feature_key = 'CUSTOM_ROLES' AND revoked_at IS NULL`, [a.businessId]);
  ...
  it('expired TRIAL (time-computed, no scheduler) loses all features (§30–31)', async () => {
    ...
    await grantFeature(a.businessId, a.userId, 'CUSTOM_ROLES');
    await ownerPool().query(
      `UPDATE business_entitlements SET trial_ends_at = now() - interval '1 day' WHERE business_id = $1`,
      [a.businessId],
    );
    // Stored state still 'trial' — the EFFECTIVE state is expired.
    const summary = await t.request.get('/v1/businesses/current/entitlement')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId);
  ...
  it('trial length comes from the VERSIONED contract (§40–44): plan_versions.trial_days drives trial_ends_at', async () => {
    ...
    const { rows } = await ownerPool().query<{ id: string }>(
      `SELECT id FROM plan_versions WHERE plan_key = 'free' AND state = 'DRAFT'`, [],
    );
    ...
    await ownerPool().query(
      `INSERT INTO plan_entitlements (plan_version_id, feature_key, enabled)
       ...
       ON CONFLICT DO NOTHING`, [draftId],
    );
    await ownerPool().query(
    ...
    const a = await onboard();
    const row = must((await ownerPool().query<{ days: string }>(
      `SELECT round(extract(epoch FROM (trial_ends_at - started_at)) / 86400)::text AS days
       FROM business_entitlements WHERE business_id = $1`, [a.businessId])).rows[0]);
