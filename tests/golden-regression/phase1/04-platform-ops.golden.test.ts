...
describe('golden: platform ops', () => {
  ...
  async function platformOwner(): Promise<{ token: string; userId: string }> {
    const reg = await t.request.post('/v1/auth/register').send({
      email: uniqueEmail(), password: 'Str0ng!Passw0rd', displayName: 'Owner', preferredLocale: 'en',
    });
  ...
  it('P1-GOLD-27 diff preview shows trial/feature/limit changes', async () => {
    const owner = await platformOwner();
    const clone = await t.request
      .post('/v1/admin/plan-versions')
      .set('Authorization', `Bearer ${owner.token}`)
      .send({ planKey: 'free', trialDays: 45 });
    expect(clone.status).toBe(201);
    const diff = await t.request
      .get('/v1/admin/plans/free/versions/diff?from=1&to=2')
      .set('Authorization', `Bearer ${owner.token}`);
  ...
  it('P1-GOLD-29 support session lifecycle: 403 → create → banner → revoke → 403', async () => {
    ...
    const session = await t.request
      ...
      .set('Authorization', `Bearer ${owner.token}`)
      .send({ tenantId, reason: 'Customer ticket #1234 — investigating order sync',
  ...
  it('P1-GOLD-30 support session terms are immutable; revocation is one-way', async () => {
    ...
    const session = await t.request
      .post('/v1/admin/support-sessions')
      .set('Authorization', `Bearer ${owner.token}`)
      .send({ reason: 'Immutable-terms check with enough length',
  ...
  it('P1-GOLD-32 bootstrap CLI: first run succeeds, second refuses, one audit event', async () => {
    ...
    const first = await execFileP(
      ...
      ['scripts/bootstrap-platform-owner.ts', `--email=${email}`, '--confirm=BOOTSTRAP'],
      { env, cwd: process.cwd() },
    );
    expect(first.stdout).toContain(email);
    const second = await execFileP(
      './node_modules/.bin/tsx',
      ['scripts/bootstrap-platform-owner.ts', `--email=${uniqueEmail()}`, '--confirm=BOOTSTRAP'],
      { env, cwd: process.cwd() },
    ).then(
      () => ({ code: 0 }),
    ...
    const audit = await ownerPool().query(
      `SELECT count(*)::int n FROM audit_events WHERE action = 'platform.owner_bootstrapped'`,
    );
