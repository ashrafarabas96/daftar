...
describe('golden: catalog & commerce', () => {
  ...
  it('P1-GOLD-22 variants carry their own SKU + price override', async () => {
    ...
    expect(res.status).toBe(201);
    const variants = await ownerPool().query('SELECT sku, price_minor FROM product_variants WHERE product_id=$1 ORDER BY sku', [res.body.id]);
