...
describe('tenant memberships', () => {
  ...
  it('onboarding creates the tenant_owner membership in the same transaction', async () => {
    ...
    const { rows } = await ownerPool().query<{ role_key: string }>('SELECT role_key FROM tenant_memberships WHERE tenant_id = $1 AND user_id = $2', [
      on.body.tenantId as string,
      me.body.userId as string,
    ]);
  ...
  it('a global user can belong to MULTIPLE tenants and MULTIPLE businesses', async () => {
    ...
    const { rows: u } = await ownerPool().query<{ id: string }>(`INSERT INTO users (email, password_hash, display_name) VALUES ($1, 'x', 'U') RETURNING id`, [
      uniqueEmail(),
    ]);
    const user = u[0];
    if (!user) throw new Error('fixture user insert failed');
    const { rows: tenants } = await ownerPool().query<{ id: string }>(`INSERT INTO tenants DEFAULT VALUES RETURNING id`);
    const t1 = tenants[0];
    ...
    const { rows: tm } = await ownerPool().query<{ n: number }>('SELECT count(*)::int AS n FROM tenant_memberships WHERE user_id = $1', [user.id]);
  ...
  it('DB rejects a membership linking tenant A to a business of tenant B', async () => {
    ...
    await ownerPool().query(`INSERT INTO tenant_memberships (tenant_id, user_id, role_key) VALUES ($1, $2, 'tenant_member'), ($3, $2, 'tenant_member')`, [
      must(tA[0], 'tenantA').id,
      must(u[0], 'user').id,
      must(tB[0], 'tenantB').id,
    ]);
    const { rows: b } = await ownerPool().query<{ id: string }>(
      `INSERT INTO businesses (tenant_id, name, store_slug, base_currency, country_code, timezone)
       VALUES ($1, 'X', $2, 'ILS', 'PS', 'Asia/Hebron') RETURNING id`,
  ...
  it('invitation accept creates tenant_member for a user from ANOTHER tenant', async () => {
    ...
    const t2 = await createTestApp({
      delivery: {
        ...
        sendInvitation: (_e: string, tok: string) => {
