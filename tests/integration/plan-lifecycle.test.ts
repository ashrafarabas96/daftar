...
async function onboard(t: TestApp, slug?: string): Promise<{ token: string; businessId: string }> {
  ...
  const on = await t.request.post('/v1/onboarding/complete')
    ...
    .send({
      businessName: 'B', countryCode: 'PS', baseCurrency: 'ILS',
...
describe('plan lifecycle & override integrity (§34–44)', () => {
  ...
  async function platformOwner(): Promise<{ token: string; userId: string }> {
    ...
    await ownerPool().query(
      `INSERT INTO platform_role_memberships (user_id, role_key) VALUES ($1, 'platform_owner')`, [userId]);
  ...
  it('DRAFT is editable; PUBLISHED freezes the version AND its children; SUNSET only from PUBLISHED', async () => {
    ...
    const created = await t.request.post('/v1/admin/plan-versions').set('Authorization', `Bearer ${p.token}`)
    ...
    await ownerPool().query(
      `UPDATE plan_limits SET limit_value = 4 WHERE plan_version_id = $1 AND limit_key = 'MAX_USERS'`, [pvId]);
    ...
    await expect(ownerPool().query(
      `UPDATE plan_limits SET limit_value = 9 WHERE plan_version_id = $1 AND limit_key = 'MAX_USERS'`, [pvId],
    )).rejects.toThrow(/immutable/);
    await expect(ownerPool().query(
      `UPDATE plan_versions SET version = 99 WHERE id = $1`, [pvId],
    )).rejects.toThrow(/immutable/);
  ...
  it('ends_at must be after starts_at (§42)', async () => {
    ...
    await expect(ownerPool().query(
      `INSERT INTO entitlement_overrides (business_id, limit_key, limit_value, reason, actor_user_id, starts_at, ends_at)
       VALUES ($1, 'MAX_USERS', 5, 'bad-window', $2, now(), now() - interval '1 hour')`, [b, p.userId],
  ...
  it('overlapping ACTIVE overrides for the same key are rejected (§43), sequential windows allowed', async () => {
    ...
    await ownerPool().query(
      `INSERT INTO entitlement_overrides (business_id, limit_key, limit_value, reason, actor_user_id)
       VALUES ($1, 'MAX_USERS', 5, 'first', $2)`, [b, p.userId]);
    await expect(ownerPool().query(
      `INSERT INTO entitlement_overrides (business_id, limit_key, limit_value, reason, actor_user_id)
       VALUES ($1, 'MAX_USERS', 6, 'overlap', $2)`, [b, p.userId],
    ...
    await ownerPool().query(
      `UPDATE entitlement_overrides SET starts_at = now() - interval '2 hours', ends_at = now() - interval '1 hour'
       WHERE business_id = $1 AND limit_key = 'MAX_USERS'`, [b]);
    await ownerPool().query(
      `INSERT INTO entitlement_overrides (business_id, limit_key, limit_value, reason, actor_user_id)
       VALUES ($1, 'MAX_USERS', 6, 'second-window', $2)`, [b, p.userId]);
  });

  it('revoke command: audited, revoked override stops applying, re-revoke → 409', async () => {
    const b = await businessOf();
    const p = await platformOwner();
    const created = await t.request.post('/v1/admin/entitlement-overrides').set('Authorization', `Bearer ${p.token}`)
    ...
    const revoke = await t.request.post(`/v1/admin/entitlement-overrides/${overrideId}/revoke`)
      .set('Authorization', `Bearer ${p.token}`).send({ reason: 'no longer needed' });
    expect(revoke.status).toBe(200);

    // Revoked override no longer wins.
    const row = must((await ownerPool().query<{ revoked_at: string | null }>(
      'SELECT revoked_at FROM entitlement_overrides WHERE id = $1', [overrideId])).rows[0]);
    expect(row.revoked_at).not.toBeNull();

    const again = await t.request.post(`/v1/admin/entitlement-overrides/${overrideId}/revoke`)
      .set('Authorization', `Bearer ${p.token}`).send({ reason: 'again' });
    expect(again.status).toBe(409);

    const audit = await ownerPool().query(
      `SELECT 1 FROM audit_events WHERE action = 'admin.entitlement_override_revoked' AND entity_id = $1`, [overrideId]);
...
describe('Gate A contract integrity (§43–51)', () => {
  ...
  it('§43: trial_days on a PUBLISHED version is immutable (row freeze covers the new column)', async () => {
    ...
    await expect(
      ownerPool().query('UPDATE plan_versions SET trial_days = 99 WHERE id = $1', [must(rows[0]).id]),
    ).rejects.toThrow();
  });
  ...
  it('§50–51: audit + outbox rows with mismatched tenant/business are REJECTED; platform NULL business allowed', async () => {
    ...
    const tA = must((await ownerPool().query<{ tenant_id: string }>(
      'SELECT tenant_id FROM businesses WHERE id = $1', [a.businessId])).rows[0]).tenant_id;
    // Mismatched pair: tenant A with business B.
    await expect(ownerPool().query(
      `INSERT INTO audit_events (tenant_id, business_id, action, entity) VALUES ($1, $2, 'x', 'y')`,
      [tA, b.businessId],
    )).rejects.toThrow();
    await expect(ownerPool().query(
      `INSERT INTO outbox_events (tenant_id, business_id, type, payload) VALUES ($1, $2, 'x', '{}')`,
      [tA, b.businessId],
    )).rejects.toThrow();
    // Matching pair passes; platform event with NULL business passes.
    await ownerPool().query(
      `INSERT INTO audit_events (tenant_id, business_id, action, entity) VALUES ($1, $2, 'ok', 'y')`,
      [tA, a.businessId],
    );
    await ownerPool().query(
      `INSERT INTO outbox_events (tenant_id, business_id, type, payload) VALUES (NULL, NULL, 'platform.event', '{}')`, [],
    );
  });
});
...
describe('§19–22: future-safe plan version immutability + clone (Terminal Closure)', () => {
  it('published version: trial_days/feature/limit changes reject; pure sunset allowed; sunset+mutation rejects', async () => {
    ...
    await expect(ownerPool().query('UPDATE plan_versions SET trial_days = 7 WHERE id = $1', [pvId]))
      .rejects.toThrow(/immutable/);
    // change feature (child) → reject
    await expect(ownerPool().query(
      `UPDATE plan_entitlements SET enabled = NOT enabled
       WHERE plan_version_id = $1 AND feature_key = (SELECT feature_key FROM plan_entitlements WHERE plan_version_id = $1 ORDER BY feature_key LIMIT 1)`, [pvId],
    )).rejects.toThrow(/immutable/);
    // change limit (child) → reject
    await expect(ownerPool().query(
      `UPDATE plan_limits SET limit_value = limit_value + 1
       WHERE plan_version_id = $1 AND limit_key = (SELECT limit_key FROM plan_limits WHERE plan_version_id = $1 ORDER BY limit_key LIMIT 1)`, [pvId],
    )).rejects.toThrow(/immutable/);
    // sunset + trial_days mutation → reject (future-safe whole-row check)
    await expect(ownerPool().query(
      `UPDATE plan_versions SET state = 'SUNSET', trial_days = 7 WHERE id = $1`, [pvId],
    )).rejects.toThrow(/immutable/);
    // pure sunset → allow
    await ownerPool().query(`UPDATE plan_versions SET state = 'SUNSET' WHERE id = $1`, [pvId]);
    const after = must((await ownerPool().query<{ state: string }>(
      'SELECT state FROM plan_versions WHERE id = $1', [pvId])).rows[0]);
    expect(after.state).toBe('SUNSET');
    // SUNSET is terminal.
    await expect(ownerPool().query(`UPDATE plan_versions SET state = 'PUBLISHED' WHERE id = $1`, [pvId]))
      .rejects.toThrow(/immutable/);
  });
  ...
  it('cloning a version copies trial_days (and features/limits) into the new DRAFT', async () => {
    ...
    try {
      ...
      const ins = await ownerPool().query(
        `INSERT INTO platform_role_memberships (user_id, role_key) VALUES ($1, 'platform_owner')`, [me.body.userId]);
      expect(ins.rowCount).toBe(1);
      expect(must((await ownerPool().query<{ role_key: string }>(
        'SELECT role_key FROM platform_role_memberships WHERE user_id = $1', [me.body.userId])).rows[0]).role_key,
      ).toBe('platform_owner');
      const created = await t.request.post('/v1/admin/plan-versions')
        .set('Authorization', `Bearer ${reg.body.accessToken as string}`)
        .send({ planKey: 'free', trialDays: 30 });
      expect(created.status).toBe(201);
      const pvId = (created.body as { planVersionId: string }).planVersionId;
      const row = must((await ownerPool().query<{ trial_days: number; state: string }>(
        'SELECT trial_days, state FROM plan_versions WHERE id = $1', [pvId])).rows[0]);
      expect(row.state).toBe('DRAFT');
      expect(row.trial_days).toBe(30); // cloned base (14) then draft trial override applied
      const children = must((await ownerPool().query<{ n: number }>(
        `SELECT (SELECT count(*) FROM plan_entitlements WHERE plan_version_id = $1)
              + (SELECT count(*) FROM plan_limits WHERE plan_version_id = $1) AS n`, [pvId])).rows[0]);
  ...
  it('§XLVII: plan version diff preview — features/limits/trial changes visible between versions', async () => {
    ...
    try {
      ...
      const before = must((await ownerPool().query<{ feature_key: string }>(
        `SELECT feature_key FROM plan_entitlements pe JOIN plan_versions pv ON pv.id = pe.plan_version_id
         WHERE pv.plan_key = 'free' ORDER BY pv.version DESC, pe.feature_key LIMIT 1`)).rows[0]);
      const flip = must((await ownerPool().query<{ enabled: boolean }>(
        `SELECT pe.enabled FROM plan_entitlements pe JOIN plan_versions pv ON pv.id = pe.plan_version_id
         WHERE pv.plan_key = 'free' AND pe.feature_key = $1 ORDER BY pv.version DESC LIMIT 1`, [before.feature_key])).rows[0]);
      const created = await t.request.post('/v1/admin/plan-versions').set(auth)
      ...
      const diff = await t.request
        .get(`/v1/admin/plans/free/versions/diff?from=${newVersion - 1}&to=${newVersion}`)
        .set(auth);
      ...
      const same = await t.request
        .get(`/v1/admin/plans/free/versions/diff?from=${newVersion}&to=${newVersion}`)
        .set(auth);
