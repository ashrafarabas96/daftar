...
describe('media', () => {
  ...
  beforeEach(async () => {
    ...
    const on = await t.request
      ...
      .send({
        businessName: 'Media Co',
        countryCode: 'PS',
        baseCurrency: 'ILS',
        storeSlug: `media-${Date.now()}-${Math.floor(Math.random() * 1e6)}`,
      });
  ...
  it('valid png upload: re-encoded, scoped server-generated key, variants created, metadata stripped', async () => {
    const png = await sharp({
      ...
      .png()
      .withMetadata({ exif: { IFD0: { Copyright: 'secret-owner-data' } } })
      .toBuffer();
    ...
    const product = await t.request
      ...
      .set(auth())
      .send({
        translations: { ar: 'مصوّر' },
        basePriceMinor: '100',
        priceCurrency: 'ILS',
      });
    const attach = await t.request.post(`/v1/catalog/products/${product.body.id as string}/media/${res.body.id as string}`).set(auth());
  ...
  it('extension/MIME mismatch rejected (declared png, content jpeg)', async () => {
    const jpg = await sharp({ create: { width: 10, height: 10, channels: 3, background: { r: 1, g: 1, b: 1 } } })
      .jpeg()
      .toBuffer();
  ...
  it('malicious filename never reaches the storage key or URL', async () => {
    const png = await sharp({ create: { width: 10, height: 10, channels: 3, background: { r: 1, g: 1, b: 1 } } })
      .png()
      .toBuffer();
    const res = await t.request.post('/v1/catalog/media').set(auth()).attach('file', png, { filename: '../../../etc/passwd.png', contentType: 'image/png' });
    expect(res.status).toBe(201);
