...
describe('business timezone / locale / country registry (§63–68)', () => {
  ...
  async function onboard(payload: Record<string, unknown> = {}): Promise<{ token: string; businessId: string }> {
    ...
    const on = await t.request.post('/v1/onboarding/complete').set('Idempotency-Key', `idem-${Date.now()}-${Math.floor(Math.random()*1e9)}`).set('Authorization', `Bearer ${token}`).send({
      businessName: 'Z', countryCode: 'PS', baseCurrency: 'ILS',
      storeSlug: `tz-${Date.now()}-${Math.floor(Math.random() * 1e6)}`, ...payload,
    });
  ...
  it('PS onboarding defaults timezone to Asia/Hebron; TR → Europe/Istanbul (pack suggestion)', async () => {
    ...
    const psBiz = await t.request.get('/v1/businesses/current')
      .set('Authorization', `Bearer ${ps.token}`).set('X-Business-Id', ps.businessId);
    expect(psBiz.body.timezone).toBe('Asia/Hebron');

    const tr = await onboard({ countryCode: 'TR', baseCurrency: 'TRY', preferredLocale: 'tr' });
    const trBiz = await t.request.get('/v1/businesses/current')
      .set('Authorization', `Bearer ${tr.token}`).set('X-Business-Id', tr.businessId);
  ...
  it('timezone is user-editable (settings PATCH), invalid IANA rejected 400', async () => {
    ...
    const ok = await t.request.patch('/v1/businesses/current')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId)
      .send({ timezone: 'Asia/Amman' });
    expect(ok.status).toBe(200);
    const bad = await t.request.patch('/v1/businesses/current')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId)
      .send({ timezone: 'Mars/Olympus_Mons' });
    expect(bad.status).toBe(400);
    const cur = await t.request.get('/v1/businesses/current')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId);
  ...
  it('DB boundary rejects invalid IANA timezone even via SQL (trigger)', async () => {
    ...
    await expect(ownerPool().query(
      'UPDATE businesses SET timezone = $2 WHERE id = $1', [a.businessId, 'Not/AZone'],
  ...
  it('arbitrary country codes rejected: API 400 + DB FK (§66)', async () => {
    ...
    const res = await t.request.post('/v1/onboarding/complete').set('Idempotency-Key', `idem-${Date.now()}-${Math.floor(Math.random()*1e9)}`)
  ...
  it('every branch gets a default warehouse (§68); per-branch default uniqueness', async () => {
    ...
    await ownerPool().query(
      `INSERT INTO entitlement_overrides (business_id, feature_key, enabled_value, reason, actor_user_id)
       VALUES ($1, 'MULTI_BRANCH', true, 'tz-test', $2)`, [a.businessId, userId]);
    await ownerPool().query(
      `INSERT INTO entitlement_overrides (business_id, limit_key, limit_value, reason, actor_user_id)
       VALUES ($1, 'MAX_BRANCHES', 5, 'tz-test', $2)`, [a.businessId, userId]);

    const br = await t.request.post('/v1/businesses/current/branches')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId)
      .send({ name: 'Second' });
    expect(br.status).toBe(201);
    const wh = await t.request.get('/v1/businesses/current/warehouses')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId);
  ...
  it('locale separation: user UI locale vs business operational vs storefront (§64)', async () => {
    ...
    await t.request.patch('/v1/businesses/current')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId)
      .send({ defaultLocale: 'en', storefrontLocale: 'ar', enabledLocales: ['ar', 'en'] });
    const biz = await t.request.get('/v1/businesses/current')
      .set('Authorization', `Bearer ${a.token}`).set('X-Business-Id', a.businessId);
