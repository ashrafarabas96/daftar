...
> **أرقام المراحل مُطابِقة للخارطة المعتمدة (16 مرحلة) في `DAFTAR_IMPLEMENTATION_ROADMAP.md` بعد تطبيعها في 2026-09-21.**
...
| R-03 | Inventory | بيع متزامن لآخر قطعة يسبب مخزونًا سالبًا | عالٍ | عالٍ | خصم ذرّي مشروط في DB + اختبار سباق (GOLD-19) | مصمَّم — Phase 3 |
...
| R-06 | Offline Sync | فقدان/تكرار عمليات Offline أو تعارضات مخزون | عالٍ | عالٍ | idempotency scoped + لا حذف قبل التأكيد + سياسة offline_oversell_exception الموحّدة (M) + GOLD-25 | مصمَّم — Phase 7 |
...
| R-08 | AI | تنفيذ مالي خاطئ أو injection أو تخمين كيان | متوسط | حرج | Typed Tools + Confirmation إلزامي + لا SQL + صلاحيات المستخدم + اختبارات حقن | مصمَّم — Phase 12 |
...
| R-10 | Scaling | تحمّل 10k+ حساب دون إثبات | متوسط | عالٍ | Load tests بأهداف قياسية + مراقبة p95 + خطة توسع موثقة | مفتوح — Phase 15 |
...
| R-14 | Storefront | بطء المتجر يقتل التحويل | متوسط | متوسط | SSR/CDN/صور محسّنة + مراقبة LCP كبوابة | Phase 6 |
