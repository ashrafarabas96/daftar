...
- `reason TEXT NOT NULL` with a non-empty CHECK; actor per AL-04; `entry_date` defaults to today.
  - **Superseded by a P2-S4 Tech Lead decision (2026-09-22).** `entry_date` has **no default** for a reversal: the merchant states it, and `accounting_post_reversal` refuses `NULL` with `accounting.entry_date_required` rather than resolving one from the business clock. The reason the earlier default could not stand: the `acctfp/1` fingerprint covers the entry date, and a reversal carries no idempotency key because the original entry id *is* its source identity — so a server-chosen date makes the command's signed identity a function of when the request arrived, and a byte-identical retry across local midnight is refused as `accounting.reversal_exists`. The bounds in the date-policy table below are unchanged: `entry_date ≥ the original entry's date` and `entry_date ≤ today` in the business timezone. The clock still bounds the stated date; it never chooses it. Recorded here rather than rewritten, so both the original decision and the correction stay readable.
...
Other properties, unchanged: `CREATE UNIQUE INDEX ... ON accounting_opening_balances (business_id) WHERE status = 'posted'` gives exactly one current set per business **at any instant**, and makes replacement impossible without first superseding. It is a bound on what STANDS, never on how many opening balances a business has posted over its life: the lifecycle is `draft → posted → reversal → superseded`, so a business may post a second set once the first has been reversed and superseded. P2-S6's closed-history rule is what stops that lifecycle from reaching behind closed books; the equity plug is an explicit visible line to the `opening_equity` system account, never an invisible adjustment; foreign positions carry a complete FX snapshot with `fx_rate_source='manual'`; `source_type='opening_balance'`, `source_id = accounting_opening_balances.id`; and it posts through `accounting_post_entry` like everything else.
...
**Periods are NOT in the first implementation slice.** They are slice **P2-S6**, confirmed and directed by the Tech Lead and now implemented as migration `0049`, accepted and frozen. Nothing in P2-S1…P2-S5 needs a closed period to be correct, and designing close/reopen concurrency before any posting traffic exists would have been speculation. `journal_entries.entry_date DATE NOT NULL` ships in P2-S2, and adding periods later required **no journal schema change**.
...
> **Delivered in P2-S6 (migration `0049`, accepted and frozen).** Every item above, plus five the lock did not ask for and review required: an append-only operation registry that decides an idempotent replay BEFORE any state is read (without it, an old reopen replayed after a later re-close reopens the period again); a single lock order — business, then period — obeyed by the posting path as well as the commands, which is what makes close-versus-post a race with a determined winner rather than a deadlock; an ACTIVATION model, because the lock's wording assumed periods would simply exist; and the opening-balance exception in the row above, which Tech Lead review found missing. The coverage rule as first written made a legitimate historical opening balance depend on whether the merchant had defined a period first — the same fact accepted in one setup order and refused in the other. The exception is source-specific, strictly before the earliest start, and proved in both race orders. And fifth, the CLOSED-BOOKS TOPOLOGY, which the second review round required: the lock asked for "closed-period posting refused in the database", and that sentence is true and insufficient, because it says nothing about an OPEN period held behind a CLOSED one, about a new earlier period created behind closed books, or about a new opening position dated before them — three ways to change balances already declared final without ever writing a row inside a closed period. So, per business and sorted by `start_date`, the periods are **zero or more CLOSED followed by zero or more OPEN**, contiguous, enforced physically by `accounting_period_topology_check()` on a DEFERRABLE INITIALLY DEFERRED constraint trigger, which no writer can bypass and which the schema owner is refused exactly as a merchant is. Close runs oldest to newest (`accounting.period_close_order`), reopen newest to oldest (`accounting.period_reopen_order`), no period may be created before the earliest while any is closed (`accounting.period_prepend_closed_history`), and no NEW entry may be dated before the earliest period while any is closed (`accounting.period_closed_history`). Neither close nor reopen ever cascades. Evidence: `docs/PHASE_2_S6_ACCEPTANCE.md`.
...
| **P2-S2** | Journal + binding **structural schema only**: `journal_entries`, `journal_lines`, `accounting_source_types`, `accounting_source_bindings`, all CHECKs, both immutability triggers, both deferred validation triggers, RLS, **and the full REVOKE shape**. **No writer function. No EXECUTE granted to anyone.** | `0042`, `0043` | no runtime principal can write to these tables directly — not `daftar_app`, not `daftar_platform`, not any runtime role — and no writer primitive exists yet (the deployment migrator remains a separate deployment trust boundary). The invariants are already active before the first row can exist | Matrix 1 (privilege, generated from the live catalogue) and Matrix 2 (invariants A–H) both green; guard G-2 active |
...
| **P2-S6** ✅ accepted / frozen | Accounting periods (AL-14) — confirmed and directed | `0049` | a `BEFORE INSERT` trigger on `journal_entries`, which binds every writer rather than only the callers of the validation routine, plus a DEFERRABLE constraint trigger on `accounting_periods` that judges the shape of the whole set at COMMIT | activation, overlap, contiguity, posting, transition, authorization, close↔post, reopen↔post, closed-books topology and raw-SQL invariant matrices green; `gate:phase2:s6` green |
...
| **P2-S8** ✅ accepted / frozen | Red team, cross-tenant, raw SQL, failure injection, rollback rehearsal, performance dataset, KMS-backed signer review — **and, on Tech Lead authorization during the slice, two migrations** | **2** — `0051`, `0052` (originally planned as **0**; see the note below) | `0051` creates a read-only authority and grants no write anywhere; `0052` reshapes existing policies and introduces none, asserting before and after that the same principals are admitted | all six budgets inside their ceilings at Tier 2, measured on GitHub at the accepted SHA; the twelve-case failure-injection matrix, the reconciliation matrices and the rollback rehearsal green |
...
**AL-18 evolution — P2-S8 was planned as a verification-only slice and did not stay one.** The original decision above is not withdrawn and is left standing as written: P2-S8 was scoped to *prove* the engine, not to change it, with **0 migrations** and an exit criterion of "budgets met or materialization justified". What actually happened is recorded here rather than by editing that decision away.

Two of the slice's own proofs came back negative in a way that verification alone could not answer.

1. **The reconciliation authority (`0051`).** The P2-S0 grant sketch had reconciliation reading through `daftar_worker`. Proving the stolen-credential matrix showed what that would mean: `daftar_worker` holds the credential-delivery transport, the decryption key ring and the SMTP authority, so giving it the journal would make one stolen delivery credential a financial reader. The Tech Lead chose Option B — a seventh role, `daftar_reconciler`, that reads the books and nothing else. A role and its grants are schema, so the decision required a migration. It is read-only in every direction: column-level `SELECT` on six tables, no `INSERT`/`UPDATE`/`DELETE` anywhere, no financial command, no RLS bypass, and `0051` asserts against the live catalogues that `daftar_worker` gained nothing.

2. **The RLS performance correction (`0052`).** Budget C missed its 500 ms ceiling in two independent runs. The first diagnosis — a correlated per-row `EXISTS` in the `tenant_membership` policy — was **refuted** when the measurement was repeated at acceptance scale, and it is withdrawn. The real causes were both in the policy's *shape*, not in the absence of a stored figure: a `SET search_path` clause on the three policy helpers, which makes PostgreSQL refuse to inline them so that every mention becomes a real per-row call; and `tenant_id::text = app_tenant()`, which compares an expression, for which PostgreSQL keeps no statistics, destroying the row estimate. `0052` replaces the three helpers with SQL-standard bodies — strictly stronger than pinning a path, because such a body is resolved at CREATE time and leaves no name a caller's `search_path` can reach — and reshapes six policies to compare the column directly. The Tech Lead authorized widening it from one policy to six on 2026-09-24, because every partial correction measured slower than no correction at all.

**What this does NOT mean.** It is not a licence for a verification slice to grow schema when a proof is inconvenient. Both migrations were authorized explicitly by the Tech Lead, during the slice, each as an answer to a negative result the slice itself produced; both were held as candidates and corrected in place through review; neither adds a stored, cached or materialized accounting figure, so AL-15 stands untouched; and the exit criterion was met the way AL-18 intended — the budgets were met, and no materialization was justified or added. Both are frozen at the digests they were accepted at (`frozenThrough` = `0052_accounting_journal_lines_rls_performance.sql`, 53 frozen migrations), and `npm run gate:phase2:s8` carries both digests independently of the manifest.

**P2-S9 remains 0 migrations.** Release closure changes no schema. If closure appears to require one, that is a finding to report, not a migration to write.
