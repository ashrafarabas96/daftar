...
| **P2-S4** ✅ implemented, pending Tech Lead review | Reversal, manual adjustment, opening balance — Phase-2-owned sources only | `0046`, `0047` (candidates — not yet frozen) | AL-12 and AL-13 state-machine tests green; the source, concurrency, authority and upgrade matrices green; guard G-4 widened to protect **every** journal writer; `npm run gate:phase2:s4` green. Evidence: `PHASE_2_S4_ACCEPTANCE.md` |
| **P2-S5** (not authorized) | FX foundation: manual rate source, immutable snapshot, rounding, realized-FX primitive | `0048` | the seven AL-09 vectors green in both implementations |
