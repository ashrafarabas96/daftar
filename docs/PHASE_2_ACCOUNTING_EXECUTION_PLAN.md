...
- A business gets **at most one posted opening balance**. It is the only entry allowed to predate the first accounting period, and in P2-S6 that is enforced in the database rather than assumed: the posting guard lets an `opening_balance` whose date is strictly before the earliest period start post without a covering period, and lets nothing else do so.
