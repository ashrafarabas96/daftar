...
| **P2-S6** ⏳ **CANDIDATE — awaiting Tech Lead review** | Accounting periods: a per-business range that can be closed, after which the DATABASE refuses a posting dated inside it. Confirmed and directed by the Tech Lead. | `0049` (**candidate — not frozen**; `frozenThrough` stays `0048_accounting_fx_rates.sql`, 49 frozen migrations, no `0050`) | the §9 activation model green (zero periods change nothing; the first period activates); the overlap, contiguity, posting, transition and authorization matrices green; the close-versus-post and reopen-versus-post races green in both directions with no deadlock; the `acctperiod/1` parity matrix green; the `0048 → 0049` upgrade and non-superuser portability matrices green; `npm run gate:phase2:s6` green, composing every predecessor. Evidence: `PHASE_2_S6_ACCEPTANCE.md` |
