...
| **P2-S2** ✅ **ACCEPTED / FROZEN** | Journal + binding **structural schema only**: tables, CHECKs, immutability triggers, both validation triggers, RLS and the full REVOKE shape. **No writer function, no EXECUTE granted** | `0042`, `0043` (frozen; `frozenThrough` = `0043_accounting_invariants.sql`) | Matrix 1 (privilege, all six roles) **and** Matrix 2 (invariants A–H, schema owner) both green; guards G-1 and G-2 active. Evidence: `PHASE_2_S2_ACCEPTANCE.md`. `npm run gate:phase2:s2` is now a **permanent** regression gate and runs as P2-S3's predecessor |
| **P2-S3** ✅ **ACCEPTED / FROZEN** | Assertion keys, assertion verification, `accounting_post_entry`, canonical fingerprint in both languages, the posting service, audit + outbox — **and only now `GRANT EXECUTE`** | `0044`, `0045` (frozen; `frozenThrough` = `0045_accounting_post_entry.sql`) | AL-03 spoofing suite, AL-11 parity + idempotency matrix, AL-17 atomicity cases, the §41–§43/§50/§51 concurrency matrices and the §30 engine-shape goldens all green; guard G-4 active; `npm run gate:phase2:s3` green and now a **permanent** regression gate, running as P2-S4's predecessor. Evidence: `PHASE_2_S3_ACCEPTANCE.md` |
| **P2-S4** ✅ **ACCEPTED / FROZEN** | Reversal, manual adjustment, opening balance — Phase-2-owned sources only | `0046`, `0047` (frozen; `frozenThrough` = `0047_accounting_opening_balances.sql`, 48 frozen migrations) | AL-12 and AL-13 state-machine tests green; the source, concurrency, authority and upgrade matrices green; guard G-4 widened to protect **every** journal writer; `npm run gate:phase2:s4` green and now a **permanent** regression gate, running as P2-S5's predecessor. Evidence: `PHASE_2_S4_ACCEPTANCE.md` |
| **P2-S5** ✅ **ACCEPTED / FROZEN** | FX foundation: an append-only MANUAL rate history per business, a deterministic lookup that infers nothing, and the pure realized-FX and rounding primitives (which decide an account and post nothing) | `0048` (frozen; `frozenThrough` = `0048_accounting_fx_rates.sql`, 49 frozen migrations) | the seven AL-09 vectors green in both implementations; the §46 lookup matrix, the `acctctl/1` authority and tamper matrices, the two-connection concurrency matrix, the HTTP contract matrix, the `fxrate/1` parity matrix, and the upgrade and non-superuser portability matrices all green; `npm run gate:phase2:s5` green and now a **permanent** regression gate, running as P2-S6's predecessor. Accepted at head `ff382719de5e1e7a20999cc9db9e45ecceea2066`, exact-SHA workflow 35791858358. Evidence: `PHASE_2_S5_ACCEPTANCE.md` |
| **P2-S6** ✅ **ACCEPTED / FROZEN** | Accounting periods: a per-business range that can be closed, after which the DATABASE refuses a posting dated inside it, and — from review round 2 — a CLOSED-prefix/OPEN-suffix topology enforced by a deferred constraint trigger | `0049` (frozen; `frozenThrough` = `0049_accounting_periods.sql`, 50 frozen migrations) | the §9 activation model green (zero periods change nothing; the first period activates); the overlap, contiguity, posting, transition and authorization matrices green; the closed-books topology, close/reopen order and prepend matrices green; the cross-slice opening-balance replacement lifecycle proved end to end; the close-versus-post and reopen-versus-post races green in both directions with no deadlock; the `acctperiod/1` parity matrix green; the `0048 → 0049` upgrade and non-superuser portability matrices green; `npm run gate:phase2:s6` green and now a **permanent** regression gate, running as P2-S7's predecessor. Accepted at head `74162f04bb5c54a918bd42441f871c1e8a05377c`, exact-SHA workflow 35852844774. Evidence: `PHASE_2_S6_ACCEPTANCE.md` |
| **P2-S7** ✅ **ACCEPTED / FROZEN** | Trial balance, general ledger, account balances, entry list and detail, account list — LIVE aggregation from the journal, with no stored, cached or materialized figure anywhere; `accounting.view` on every route, Phase 1 branch scope beneath it, keyset pagination, exact integer money | `0050` (candidate, INDEXES ONLY — two, created after the query-plan measurement showed one account's ledger page reading the whole journal) | the whole-business trial balance balances exactly or is refused; a branch-filtered report states that it is a dimension; every figure equals an INDEPENDENT recomputation from the journal rows; the permission, branch and tenant matrices green; keyset traversal proved at seven page sizes and across a concurrent append; a deactivated account keeps its history; a rate entered today does not change a posting from last year; `npm run gate:phase2:s7` green and now a **permanent** regression gate carrying `0050`'s accepted digest independently of the manifest. Accepted at head `39a7503277a315c559291c15c34b66a4f4fab301`, exact-SHA workflow 35874918898; frozen at head `548fb11d5bfed876a73e67f3346fcf85e8904f00`, exact-SHA workflow 35889849600, all five jobs SUCCESS — at the time, `frozenThrough` = `0050_accounting_report_indexes.sql` with 51 frozen migrations; the boundary has since moved to `0052` on acceptance of P2-S8. Evidence: `PHASE_2_S7_ACCEPTANCE.md` |
| **P2-S8** ✅ **ACCEPTED / FROZEN** | Security, failure, reconciliation and performance hardening: a dedicated `daftar_reconciler` authority in its own no-HTTP process that may DETECT and may never REPAIR, the twelve-case failure-injection matrix with no production debug switch, a real backup/restore/upgrade rehearsal against the accepted Phase 1 build, supply-chain hygiene, observability redaction, and the six budgets measured at acceptance scale | `0051`, `0052` (frozen; `frozenThrough` = `0052_accounting_journal_lines_rls_performance.sql`, 53 frozen migrations) | nine reconciliation checks each proved by planting what they look for, and `unavailable` never counted as success; the stolen-credential and cross-process secret matrices green; `daftar_worker` still unable to enumerate businesses or read the journal; the rollback rehearsal PASS over 12 steps; the runner proved able to report failure before any test result is trusted; **all six budgets inside their ceilings at Tier 2** (budget C 179.0 ms p50 / 205.7 ms p95 at 104,478 lines against a 500 ms ceiling). **TD-11 is CLOSED by `0052`, and its original diagnosis was refuted and withdrawn:** the per-row correlated `EXISTS` was not the cost. The two real causes were a `SET search_path` clause on the three policy helpers, which makes PostgreSQL refuse to inline them and turns every mention into a per-row call, and `tenant_id::text = app_tenant()`, which compares an expression and destroys the row estimate. `0052` replaces the three helpers with SQL-standard bodies — strictly stronger than a pinned path, because the body is resolved at CREATE time and has no name left for a caller's path to reach — and reshapes six policies to compare the column. Widening `0052` from one policy to six was authorized by the Tech Lead on 2026-09-24, because every partial correction measured SLOWER than no correction at all. Accepted at head `d4ec6c4f5be838e3c47d40719e44d3213727e566`, with `DAFTAR CI` 35966829333 (all five jobs SUCCESS) and `DAFTAR P2-S8 acceptance evidence` 35966820087 (11 pass / 0 fail / 0 skipped, release gate PASS) on that exact SHA. `npm run gate:phase2:s8` is now a **permanent** regression gate carrying both accepted digests independently of the manifest. Evidence: `PHASE_2_S8_ACCEPTANCE.md`, `PHASE_2_PERFORMANCE_BASELINE.md` |
...
> **Implemented in P2-S2** (migrations `0042`/`0043`), and completed in **P2-S3** (migrations `0044`/`0045`). What §3, §4, §6, §7, §8, §9 and §11 describe is now schema, not plan. The posting primitive named below, deliberately absent from P2-S2, landed in P2-S3: `accounting_post_entry()` exists, `daftar_accounting_internal` holds `INSERT` (never `UPDATE`, `DELETE` or `TRUNCATE`) on the three ledger tables, and the sentence "runtime roles hold `SELECT` plus `EXECUTE` on that one function" is now the live end state — with the correction that only `daftar_app` holds that EXECUTE, because platform administration is not financial authority.
...
- Grants (→ AL-03): `daftar_app` → `SELECT` on entries/lines/bindings/accounts **plus `EXECUTE` on `accounting_post_entry`**; **no `INSERT`, `UPDATE` or `DELETE` on the journal or the bindings to anyone**; `daftar_worker` → **nothing** (the P2-S0 plan said `SELECT` for reconciliation; `0042` never granted it, and P2-S8 closed the question the other way — see below); `daftar_platform` → `SELECT` plus chart management; `daftar_identity`, `daftar_resolver`, `daftar_provisioner` → **nothing**.
- **Superseded by the P2-S8 decision (Option B, Tech Lead).** Reconciliation does NOT read through `daftar_worker`. That process holds the credential-delivery transport, the credential decryption key ring and the SMTP authority, and giving it the journal would have made a stolen delivery credential a financial reader. A seventh role, `daftar_reconciler`, reads the books and nothing else — column-level SELECT on six tables, no write anywhere, no financial command, no PII, no RLS bypass — and `0051` asserts against the live catalogues that `daftar_worker` gained nothing. The P2-S0 architecture lock's Matrix 1 grant sketch is historical from this point on.
...
> **Implemented in P2-S5** (migration `0048`, accepted and frozen). What this item describes is now schema, with four things sharper in the implementation than in the sketch. The table is named `accounting_fx_rates` and carries its own `id`, so a rate has a stable identity a snapshot can cite. `source` is CHECK-pinned to the single value `'manual'` rather than left open: a `provider` value nothing can write would be a promise the schema makes and the code cannot keep. `effective_at` is constrained to whole UTC seconds, because `acctfp/1` cannot fingerprint a sub-second instant and this value is directly usable as a line's `fx_rate_at`. And "append-only" is enforced by an unconditional `BEFORE UPDATE OR DELETE` trigger as well as by the absence of the grant, so the rule survives the one principal a privilege check cannot stop. The lookup is `accounting_fx_rate_lookup`, deliberately **not** `SECURITY DEFINER`: running as the caller makes cross-business isolation row level security's answer rather than the function's. Evidence: `PHASE_2_S5_ACCEPTANCE.md`.
...
> **Foundation implemented in P2-S5.** `classifyRoundingResidual` (`packages/accounting/src/rounding.ts`) is the primitive this rule needs: its only account is `rounding`, the balancing side is stated by the caller rather than guessed, the magnitude is bounded by the allocation count — an unbounded residual is refused with `accounting.rounding_residual_unbounded` — and a non-empty reason is mandatory. Its twin, `classifyRealizedFx`, returns `fx_gain`/credit or `fx_loss`/debit, and neither function can return the other's account or `purchase_price_variance`: INV-ACC-12's three-way separation is a property of the return types, proved exhaustively rather than reviewed. Both DECIDE and neither POSTS — the domain that settles a payment does that.
...
> **Implemented in P2-S4** (migration `0047`). What this section describes is now schema, not plan — with two corrections to the sketch below, both deliberate. First, the tables are named `accounting_opening_balances` and `accounting_opening_balance_lines`, matching the slice's prefix convention rather than the bare names here. Second, the status enum is `draft | posted | superseded`, not `draft | posted`: AL-13 requires a third state, because replacing a posted set must leave the old one visible as a superseded fact rather than deleting it. The balancing account is addressed by its `system_key` `opening_equity`, never by the code `3000`, per AL-07. The sentence about predating the first period is no longer plan either: P2-S6 implements it as a rule in the posting guard, described in §16.
...
- A business may have **several opening-balance sets over its history, and at most one CURRENT set with `status = 'posted'` at any instant**. Earlier sets remain as `superseded` history after a valid reversal — **lifetime count is not current posted count**, and the partial unique index bounds the second, never the first. (An earlier revision of this page said "at most one posted opening balance" without that distinction; the sentence is corrected rather than deleted, because a reader who took it as a lifetime cap would reason about the wrong risk.) The opening balance is the only entry allowed to predate the first accounting period, and in P2-S6 that is enforced in the database rather than assumed: the posting guard lets an `opening_balance` whose date is strictly before the earliest period start post without a covering period, and lets nothing else do so — and refuses even that while any period of the business is closed. Restating the opening position once the books have periods therefore runs the whole AL-13 lifecycle through the real commands: reopen the closed periods newest-first, reverse the current set's journal on a date inside an OPEN period, post the replacement, which supersedes the old set. Proved end to end in `tests/integration/accounting-periods-closed-books.test.ts`.
...
- `accounting_periods (id, tenant_id, business_id, start_date, end_date, status [open|closed], closed_by, closed_at)`, non-overlapping, contiguous, and of **whatever length the merchant states**.
  > **Superseded sketch.** This line originally ended "monthly by default". There is no monthly default and no inferred frequency: DAFTAR does not know a merchant's fiscal calendar and does not guess one. A merchant explicitly creates monthly, quarterly, four-week, weekly, single-day or any other finite contiguous ranges, subject only to the topology rules. The implemented columns are `start_date`/`end_date`, and non-overlap is a physical `EXCLUDE USING gist` constraint rather than the `UNIQUE(business_id, starts_on)` sketched here, which would have permitted two periods starting on different days and overlapping anyway.
...
- Closing a period is a domain command: it locks the period row, verifies the period's entries balance, writes an audit record and emits an outbox event. **P2-S6 adds a precondition the sketch did not state:** a period may be closed only when no EARLIER period is still open, and reopened only when no LATER period is still closed. Neither command touches a period the merchant did not name. Phase 2 does **not** implement year-end income-statement closing entries (revenue/expense → equity) — that requires a retained-earnings policy decision, recorded in §36.
...
- **Placement (→ AL-14): periods are NOT in the first implementation slice.** They are slice **P2-S6**, now implemented, accepted and frozen. `journal_entries.entry_date` ships in P2-S2, and the plug-in point turned out to be a `BEFORE INSERT` trigger on `journal_entries` rather than the AL-02 validation routine — which needed **no journal schema change either**, and has the advantage that the refusal binds every writer including a direct `INSERT`, not only the paths that call the routine.

> **Implemented in P2-S6** (migration `0049`, accepted and frozen). Three things are sharper in the implementation than in the sketch. First, periods do NOT become the authoritative posting-date gate for everyone: they become it for a business that has created one. A business with zero periods keeps exactly the rules it had, because the alternative is a migration that invents a fiscal calendar and starts refusing postings for merchants who never asked for periods. Second, the universal no-future rule is unchanged and still lives in the frozen P2-S3 writer; a period narrows what may be posted and never widens it. Third, one source-specific exception exists, and it is the §15 sentence made enforceable: an `opening_balance` dated strictly before the earliest period start posts without a covering period. Without it the same opening position would be accepted or refused depending on whether the merchant defined a period first, which is an accounting answer that depends on the order somebody configured the product. The exception goes no further than that — an opening balance inside a closed period is refused like any other entry, one after the chain is refused for want of coverage, and no other source has it.

> Fourth, and required by the second review round: a close is a statement about the BOOKS, not a filter on a date range. "Refuse an entry dated inside a closed period" leaves three ways to change balances already declared final without writing any row inside a closed period — hold an open period behind a closed one, create a new earlier period behind closed books, or state a new opening position dated before them. So per business, sorted by `start_date`, the periods are zero or more CLOSED followed by zero or more OPEN, contiguous, enforced by `accounting_period_topology_check()` on a DEFERRABLE INITIALLY DEFERRED constraint trigger that judges the whole set at COMMIT. Closing runs oldest to newest, reopening newest to oldest, neither cascades, no period may be created before the earliest while any is closed, and no new entry may be dated before the earliest period while any is closed. Evidence: `PHASE_2_S6_ACCEPTANCE.md`.
...
| `accounting.period.manage` | Create and close a period. **Sensitive.** Registered in P2-S6 (→ AL-14). It does NOT imply `accounting.period.reopen`. |
| `accounting.period.reopen` | Reopen a closed period, and nothing else. **Sensitive.** Registered in P2-S6. Undoing a close is a separate decision from making one, and there is no fallback between the two keys at any layer. |
...
- **Registered in P2-S1 (migration `0041`):** `accounting.view`, `accounting.post`, `accounting.reverse`, `accounting.chart.manage`, `accounting.fx.manage`. **Registered in P2-S6 (migration `0049`):** `accounting.period.manage` and `accounting.period.reopen`, both SENSITIVE, backfilled onto every existing owner role and onto no other role. The registry shipped no dead keys: each one arrived with the slice that could use it.
...
> **Implemented in P2-S4** (migration `0046`). What this section describes is now schema, not plan. One thing below is sharper in the implementation than in the sketch: the mirror lines are **derived by the database from the persisted original**, and `accounting_post_reversal` takes no line parameter at all — so "the original lines with debit and credit exchanged" is not a rule the caller is trusted to follow, it is the only thing the routine can write. One thing is weaker: "a reversal into a closed period is refused" became true in P2-S6, and not by a rule about reversals: the `BEFORE INSERT` guard on `journal_entries` refuses EVERY writer’s entry dated inside a closed period, so a reversal is covered because it is a posting, not because anybody remembered to name it.
...
| Concurrent FX rate entry | Unique `(business_id, from, to, effective_at)`, plus two narrow advisory locks taken in a fixed order (rate identity, then pair-and-instant). **P2-S5 correction to the sketch:** a duplicate is only a no-op when the rate is the SAME. Two different rates for one pair at one instant are two different answers to one question, and the second is refused with `accounting.fx_rate_conflict` rather than silently ignored or allowed to overwrite. |
...
| 0044 | P2-S3 ✅ | `accounting_assertion_keys` | Assertion key table and `jti` replay registry with no grants to any runtime role, install/retire commands granted to `daftar_platform` only (→ AL-03). |
| 0045 | P2-S3 ✅ | `accounting_post_entry` | `accounting_actor()` verification, the `acctfp/1` canonicalizer (`BYTEA`) and fingerprint, the narrow SECURITY DEFINER writer with in-database fingerprint recomputation, the date policy read as data, idempotency, audit + outbox in the same transaction, the `financial_started_at` transition and its guard, **and the single `GRANT EXECUTE` to `daftar_app`** (→ AL-03, AL-11, AL-14, AL-17). |
...
| 0048 | P2-S5 | `accounting_fx_rates` | Append-only manual rate history + the deterministic lookup, plus the `acctctl/1` control-assertion verifier and the `fxrate/1` canonicalizer (→ AL-09). Entering a rate posts nothing and registers no source type. |
| 0049 | P2-S6 | `accounting_periods`, `accounting_period_operations` | **Frozen** (→ AL-14). The period table with a real gist `EXCLUDE` for non-overlap, the append-only operation registry behind idempotent create/close/reopen, and the closed-period refusal as a `BEFORE INSERT` trigger on `journal_entries` — no journal schema change. Creating a period posts nothing and registers no source type. |
| 0050 | P2-S7 | `accounting_report_indexes` | Two reporting indexes, created because `tests/performance/accounting-read-plans.test.ts` showed a single account's ledger page reading every line and every entry of the business. **ACCEPTED and FROZEN**. (The boundary has since moved to `0052`; 53 frozen migrations.) |
| 0051 | P2-S8 | `accounting_reconciler_read` | The `daftar_reconciler` authority: one narrow SECURITY DEFINER enumerator returning tenant and business ids only, column-level SELECT grants on six tables, no write anywhere and no financial command. **ACCEPTED and FROZEN** at `2086c875…7587cc`. |
| 0052 | P2-S8 | `accounting_journal_lines_rls_performance` | The RLS performance correction: the three policy helpers (`app_tenant`, `app_business`, `app_bypass`) become SQL-standard bodies so the planner can inline them, and six policies compare `tenant_id` directly instead of a cast expression. Reshapes existing boundaries; introduces no policy, no index, no grant. It snapshots which principals each policy half admits before its first statement and re-derives the same string at the end, so a dropped or added term rolls the file back. **ACCEPTED and FROZEN** at `0acf1650…72ad84`. |
| — | P2-S8 | none beyond 0052 | No materialized read model was added: the dataset did not justify one, and the budget that missed was caused by an RLS policy's SHAPE, which `0052` corrected without storing a figure anywhere (→ AL-15, TD-11 closed). |
...
| A-11 | Opening balance: at most one CURRENT opening-balance set with `status = 'posted'` per business at any instant; superseded history remains preserved (lifetime count is not current posted count); equity plug explicit; immutable once posted. |
