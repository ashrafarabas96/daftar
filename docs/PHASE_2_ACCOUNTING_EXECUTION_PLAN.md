...
| **P2-S8** 🕐 **CANDIDATE — BLOCKED** | Security, failure, reconciliation and performance hardening: a dedicated `daftar_reconciler` authority in its own no-HTTP process that may DETECT and may never REPAIR, the twelve-case failure-injection matrix with no production debug switch, a real backup/restore/upgrade rehearsal against the accepted Phase 1 build, supply-chain hygiene, observability redaction, and the six budgets measured | `0051` (candidate, the reconciler authority ONLY — not frozen, not in the manifest; no `0052`) | nine reconciliation checks each proved by planting what they look for, and `unavailable` never counted as success; the stolen-credential and cross-process secret matrices green; `daftar_worker` still unable to enumerate businesses or read the journal; the rollback rehearsal PASS over 12 steps; five of the six budgets inside their ceilings, **C (trial balance) 504.5 ms against a 500 ms ceiling, missed in two independent runs** — diagnosed to the per-row planning of the `tenant_membership` policy on `journal_lines`, which needs a schema change this slice may not make (TD-11, §36). Evidence: `PHASE_2_S8_ACCEPTANCE.md`, `PHASE_2_PERFORMANCE_BASELINE.md` |
...
| 0050 | P2-S7 | `accounting_report_indexes` | Two reporting indexes, created because `tests/performance/accounting-read-plans.test.ts` showed a single account's ledger page reading every line and every entry of the business. **ACCEPTED and FROZEN** — `frozenThrough`, 51 frozen migrations. |
| 0051 | P2-S8 | `accounting_reconciler_read` | The `daftar_reconciler` authority: one narrow SECURITY DEFINER enumerator returning tenant and business ids only, column-level SELECT grants on six tables, no write anywhere and no financial command. CANDIDATE: not frozen, not in the manifest. |
| — | P2-S8 | none beyond 0051 | No materialized read model was added: the dataset did not justify one, and the single budget that missed is caused by an RLS policy's shape, not by the absence of a stored figure (→ AL-15, TD-11). |
