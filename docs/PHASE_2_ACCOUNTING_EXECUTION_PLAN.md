...
| **P2-S5** ✅ implemented, pending Tech Lead review | FX foundation: an append-only MANUAL rate history per business, a deterministic lookup that infers nothing, and the pure realized-FX and rounding primitives (which decide an account and post nothing) | `0048` (candidate — not yet frozen) | the seven AL-09 vectors green in both implementations; the §46 lookup matrix, the `acctctl/1` authority and tamper matrices, the two-connection concurrency matrix, the HTTP contract matrix, the `fxrate/1` parity matrix, and the upgrade and non-superuser portability matrices all green; `npm run gate:phase2:s5` green. Evidence: `PHASE_2_S5_ACCEPTANCE.md` |
