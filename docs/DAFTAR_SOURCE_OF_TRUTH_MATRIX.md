...
6. ممنوع أن يُعاد قيد موجود كـ"نجاح" لطلب مالي مختلف: نفس هوية المصدر مع **حمولة مالية مختلفة جوهريًا** (أي حقل من حقول `acctfp/1`) تُرفض بـ`accounting.idempotency_conflict`. الاختلاف السردي وحده (الوصف، معرّف الطلب) هو نفس الواقعة ويُعاد بـ`created=false`، والسرد المحفوظ لا يُعاد كتابته. أول واقعة مالية مُرحَّلة هي التي تفوز (P2-S4 §29).
