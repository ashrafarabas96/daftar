...
| A-29 | A merchant variant may still be added beside an existing base variant: raw `daftar_app` SQL can insert an `is_base = false` variant on a tracked simple product that already has its base variant, and nothing in the lock states whether that is allowed, refused, or converts the product. No P3-S1 route creates variants, so it is unreachable through the API today. Raised by the P3-S1 independent security review | P3-AL-03, P3-AL-52; `0053_inventory_units_and_product_configuration.sql` (`product_variants_one_base_uq`) | once stock sits on the base variant, a second sellable variant splits the product's stock identity; S2 must decide the rule before the first movement exists |
