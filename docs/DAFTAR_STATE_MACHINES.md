...
ترتيب فترات المنشأة زمنيًّا:   closed …  closed   open …  open
مرفوض:                        open closed   |   closed open closed
```
...
- **طوبولوجيا الدفاتر المقفلة: مغلقة أولًا ثم مفتوحة، دائمًا.** فترات المنشأة مرتَّبة بـ `start_date` هي صفر أو أكثر من الفترات **المغلقة** يليها صفر أو أكثر من الفترات **المفتوحة**، بلا فجوات. الحالات `open closed` و`open closed open` و`closed open closed` ليست حالات يرفضها «دفتر» بل حالات لا يمكن أن توجد: `accounting_period_topology_check()` مُشغّل قيد **DEFERRABLE INITIALLY DEFERRED** على `accounting_periods` يُنفَّذ عند COMMIT، فالمعاملة التي تترك هذا الشكل لا تُثبَّت أصلًا — لأي كاتب كان، ومالك المخطط من ضمنهم. والتأجيل ليس تخفيفًا: القاعدتان عن شكل المجموعة كلها، وهو لا يكتمل إلا عند COMMIT.
- **الإغلاق من الأقدم إلى الأحدث، وإعادة الفتح من الأحدث إلى الأقدم.** إغلاق فترة وقبلها فترة مفتوحة يُرفض بـ `accounting.period_close_order`، وإعادة فتح فترة وبعدها فترة مغلقة تُرفض بـ `accounting.period_reopen_order`. ولا يوجد تتالٍ في أي من الاتجاهين: لا يُغلق أمرٌ فترةً لم يُسمّها التاجر، ولا يفتحها.
- **لا تاريخ جديد خلف الدفاتر المقفلة.** ما دامت **أي** فترة مغلقة: لا يجوز إنشاء فترة أقدم من أقدم فترة قائمة (`accounting.period_prepend_closed_history`)، ولا ترحيل قيد جديد مؤرَّخ قبل بداية أقدم فترة (`accounting.period_closed_history`). والرمز الثاني رمز مستقل عن `accounting.period_closed` عن قصد، لأن التاريخ يقع خارج كل الفترات فلا فترةَ مغلقةً تحتويه. من أراد تغيير وضع الافتتاح فليُعد فتح الفترات المغلقة أولًا.
- رسائل الرفض الأربع أعلاه تعيش في الأوامر لتُخاطب التاجر بجملة عن دفاتره، **وليست بديلًا عن القاعدة**: المُحقِّق المؤجَّل في قاعدة البيانات هو ما يجعل الجواب صحيحًا أمام كاتب لم يمرّ بالأوامر أصلًا.
- **استثناء واحد فقط — الرصيد الافتتاحي السابق للدفاتر.** القيد من نوع `opening_balance` المؤرَّخ **قبل بداية أقدم فترة تمامًا** يُقبل بلا فترة تغطّيه، لأن الرصيد الافتتاحي سابق لبداية الدفاتر بطبيعته. لولا هذا الاستثناء لصار الحكم على الحقيقة المحاسبية نفسها مرهونًا بترتيب التهيئة: يُقبل الرصيد إن أُدخل قبل إنشاء أول فترة، ويُرفض إن أُدخل بعدها. والاستثناء محدود من كل جانب: المقارنة «أصغر من» تمامًا فتبقى بداية أقدم فترة خاضعة لقاعدة التغطية العادية؛ وإن وقع التاريخ داخل فترة **مغلقة** فالرفض `accounting.period_closed` كأي قيد آخر؛ وإن وقع بعد سلسلة الفترات فالرفض `accounting.period_missing_for_date`؛ وإن كانت **أي** فترة من فترات المنشأة مغلقة فالرفض `accounting.period_closed_history` مهما كان التاريخ قديمًا، فلا يدخل هذا الباب شيء جديد خلف دفاتر مقفلة؛ وقاعدة منع المستقبل تبقى سارية؛ ولا يملك هذا الاستثناء أي مصدر آخر، فالتسوية اليدوية والعكس يُرفضان قبل أقدم فترة.
