...
| migration | SHA-256 |
|---|---|
| `0040_accounting_chart.sql` | `05d87a1d80a9f7895d017c93284647855ecb0378a5ac53426321f9c15a0eb63e` |
...
| Every **new** business gets its chart in the same transaction as the business row, without editing a frozen provisioning migration | `businesses_seed_chart` AFTER INSERT trigger → `accounting_seed_chart()` (SECURITY DEFINER, owner `daftar_platform`, pinned `search_path`, PUBLIC EXECUTE revoked, no runtime EXECUTE grant) | case S, and the same check through the real HTTP onboarding path |
...
| Merchant runtime holds **no** DML on the chart; identity, resolver, provisioner and worker hold nothing at all | grants in `0040`: `daftar_app` SELECT; `daftar_platform` SELECT + INSERT (the seeding routine's own authority) and nothing else | `tests/security/accounting-boundary.test.ts`, including an enumeration of `information_schema.role_table_grants` |
...
**Platform INSERT on `accounts`.** `accounting_seed_chart()` is SECURITY DEFINER owned by `daftar_platform`, which is how every provisioning routine in this schema already writes (`0032`/`0033`/`0038`), so `daftar_platform` holds `SELECT, INSERT` on `accounts` — and deliberately **not** `UPDATE` or `DELETE`. It can therefore create a chart and can never rewrite or remove one. The alternative, owning the routine by the migrator, would depend on the migrator being a superuser in every deployment. The grant shape is enumerated and asserted in `tests/security/accounting-boundary.test.ts`.
