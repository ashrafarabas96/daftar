...
| **DAFTAR does not require a SUPERUSER to migrate.** `0039` → `0040` → `0041` applies over a connection authenticated as `daftar_migrator`, a LOGIN deployment principal with `rolsuper = false` and `rolbypassrls = false` | `0040` takes CREATE on `public` only for the one statement PostgreSQL requires it for and revokes it in the same file; the backfill runs under the seeder's own identity; a closing `DO` block refuses to commit unless the end state is clean | `tests/integration/migration-portability.test.ts` — the migrations themselves execute over the non-superuser connection, not merely the assertions afterwards |
...
**Runtime authority and deployment authority are two different trust boundaries.** The first correction said `daftar_accounting_internal` is "granted to nobody". That was too strict to be true, and saying it hid a real dependency. PostgreSQL will not let a non-superuser hand a function to an owner it cannot `SET ROLE` to, so "granted to nobody" silently meant "DAFTAR requires a superuser to migrate" — not a contract a managed PostgreSQL can meet.

The boundary that actually matters is narrower: **no runtime principal may assume `daftar_accounting_internal`.** `bootstrap.sql` therefore creates one more principal, `daftar_migrator` — `LOGIN`, but `NOSUPERUSER`, `NOBYPASSRLS`, `NOCREATEROLE`, `NOCREATEDB`, `NOREPLICATION` — and grants it the single membership:

```sql
GRANT daftar_accounting_internal TO daftar_migrator WITH INHERIT FALSE, SET TRUE;
```

`SET TRUE` is what makes the ownership transfer legal without a superuser. `INHERIT FALSE` is what stops the membership from being accounting authority in its own right: it has to be assumed deliberately, and only a deployment credential can assume it. No service loads that credential and it appears in no runtime connection URL. The guard permits this one membership and only this one; any runtime member, `PUBLIC`, any other role, a missing `INHERIT FALSE` or an `ADMIN TRUE` all fail `npm run gate:phase2:s1`, and `tests/security/accounting-boundary.test.ts` asserts the live `pg_auth_members` row option-by-option.

One consequence to state plainly rather than hide: whichever principal runs the migrations **owns** the tables it creates, and PostgreSQL gives an owner full rights on them — there is no way to refuse that. So `daftar_migrator` can write `accounts` by ownership. That is exactly why it is a deployment credential and not a runtime one. The `0` in "runtime account-DML principals" is a claim about the six runtime roles, and it is asserted for each of them by name, in the migration and in the tests.

**A SUPERUSER migration was hiding a silent no-op backfill.** Writing the portability test found a defect the superuser path could not have found. `businesses` has `FORCE ROW LEVEL SECURITY`, and its policies admit a tenant-scoped session or the seeder — not a migration principal. A superuser migrator bypasses RLS and never notices. A managed, non-superuser migrator would have seen **zero** businesses, looped over nothing, and passed the backfill's own completeness check vacuously: every pre-existing business left chartless, and a green migration. `0040` now runs its backfill under `SET LOCAL ROLE daftar_accounting_internal`, so it sees exactly what the seeder is entitled to see and behaves identically under both kinds of connection.

**The temporary `CREATE` on `public`.** Requirement 3 of a non-superuser ownership transfer is that the *new owner* holds `CREATE` on the function's schema. `0040` grants it, transfers ownership, and revokes it in the same file — one transaction, so it exists in no committed state. The end state is asserted three times over: by the closing `DO` block in `0040` itself (which refuses to commit otherwise), by `authority-isolation.ts` reading the migration text, and by both test suites querying `has_schema_privilege`. The guard fails if the grant ever appears without its revoke, or if it migrates into `bootstrap.sql` as part of the role's permanent shape.

**Pre-existing Phase 1 limitation, recorded and NOT fixed here.** Migrations `0032`, `0033` and `0038` contain `ALTER FUNCTION ... OWNER TO daftar_platform`. By the same PostgreSQL rule, a non-superuser migrator can only execute those if `daftar_platform` — a **LOGIN runtime role** — holds `CREATE` on schema `public`. So the frozen `0000`–`0039` history is not yet non-superuser-portable, and the only in-place fix would grant a runtime role schema-creation rights, undoing the authority correction this slice just landed.

P2-S1 does not touch it: migrations `0000`–`0039` stay byte-for-byte frozen, and the required compatibility test is `0039` → `0041`. `tests/integration/migration-portability.test.ts` establishes the managed starting state (the migrator owning the 0039 schema, as it would in a real deployment) and then runs `0040` and `0041` over the non-superuser connection. **Deployment hardening item for a later slice:** give Phase 1's provisioning routines the same treatment — a non-login owner reached through a deployment-only membership — so the full `0000` → latest history is portable too. Evidence is the file header of that test.
...
`npm run gate:phase2:s1` (`scripts/phase2-s1-gate.ts`) refuses the tree when `0040` or `0041` is missing, any `0042+` exists, any Phase 1 migration changed, the P2-S1 migrations were frozen early, any journal/binding/posting surface appeared, the 21-key registry is incomplete or has extras, `account_translations` appeared, guard G-3 is absent or failing, **any LOGIN role holds `INSERT`/`UPDATE`/`DELETE` on the chart, anyone at all holds `UPDATE`/`DELETE` on `accounts`, either seeding routine is owned by a LOGIN role or leaves `EXECUTE` with `PUBLIC`, the internal principal gains `LOGIN`, a password, `BYPASSRLS`, `SUPERUSER`, `CREATEROLE`, `CREATEDB`, `CONNECT` or a member, the slice touches `app_bypass()`, a runtime role (or `PUBLIC`, or anyone but `daftar_migrator`) is a member of the internal principal, that one membership loses `INHERIT FALSE` or gains `ADMIN TRUE`, the migration principal is declared `SUPERUSER`/`BYPASSRLS`/`CREATEROLE` or stops asserting `NOSUPERUSER`/`NOBYPASSRLS`, or the temporary `CREATE` on `public` is taken without being given back**, or any composed check fails. It composes `check:migrations`, `check:guards`, `check:localization`, `gate:phase1`, the `@daftar/domain-core` unit tests and the six P2-S1 suites rather than duplicating them.
