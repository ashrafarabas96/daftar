...
|---|---|---|---|
| 1 — provisioner actor spoofing | `provision_actor()` read the GUC `app.actor_user_id`; a `daftar_provisioner` connection could `set_config` any user id and EXECUTE `provision_create_business` against a victim tenant | Migration `0038`: the actor is a **provisioning assertion** `v1.<kid>.<actor>.<kind>.<exp>.<jti>.<hmac-sha256>` minted by the merchant API with `PROVISIONING_ASSERTION_KEY` and verified inside `provision_actor(kinds)` with the same key stored in `provisioning_assertion_keys` — a table with **no grants at all**, so no principal (including `daftar_platform`) can read a secret or mutate it directly; key rotation happens ONLY through the narrow `provision_assertion_key_install` / `_retire` commands, whose EXECUTE is granted to `daftar_platform` alone. Bound to one operation kind, 60 s TTL, and single-transaction use with cross-transaction replay protection (`provisioning_assertion_uses`). The 0033 authority checks (tenant owner, invitation addressee, actor-scoped idempotency) stay inside the same SECURITY DEFINER commands. Keys are installed by the platform principal (`npm run bootstrap:provisioning-key`), never by migrations | `tests/security/provisioner-boundary.test.ts` "direct EXECUTE attacks (Blocker 1)": GUC spoofing, forged key, tampered claims, expired, wrong kind, replay, non-owner with valid assertion, foreign invitation, key secrets unreadable and the table directly unmutable from all six roles, `daftar_platform` install/retire ALLOWED, every other principal DENIED, and the CLI proven not to print key material; positive paths in `onboarding`, `invitation-lifecycle`, `concurrency-matrix` |
...
### Assertion replay semantics (precise)

`provisioning_assertion_uses` gives **single-transaction use with cross-transaction replay protection**, not single-command use. One provisioning workflow composes several trusted commands (for example `provision_replay_operation`, `provision_create_tenant`, `provision_create_business`, `provision_persist_operation`), and they legitimately present the SAME assertion inside the SAME transaction. The JTI is bound to the first transaction that presents it (`pg_current_xact_id()`), so a captured assertion replayed in any later transaction is refused. `provisioner-boundary.test.ts` pins both halves: several commands succeed under one assertion in one transaction, and the same assertion fails in a new one.

### Key custody (authority as PostgreSQL enforces it)

| Principal | Read key secrets | Direct INSERT/UPDATE/DELETE on the key table | `provision_assertion_key_install` / `_retire` | JTI registry |
|---|---|---|---|---|
| `daftar_platform` | DENIED | DENIED | **ALLOWED** (sole key-management principal) | DENIED |
| `daftar_provisioner` | DENIED | DENIED | DENIED | DENIED |
| `daftar_app`, `daftar_worker`, `daftar_identity`, `daftar_resolver` | DENIED | DENIED | DENIED | DENIED |

The commands return void and the rotation CLI prints only the kid, so key material never reaches a response or a log. Every row is a test in `provisioner-boundary.test.ts`.
