...
| INV-INV-14 | (P3) `value_delta_base_minor` يُقرَّب HALF_EVEN مرة واحدة عند كتابة الحركة، ثم القيمة المخزَّنة هي المرجع أبدًا؛ ولا يُعاد حساب قيمة حركة تاريخية من `qty × avg` |
...
| INV-INV-16 | (P3) لا حركة بلا سطر مصدر حقيقي ولا سطر مصدر منتهٍ بلا حركاته — يُتحقَّق عند `COMMIT` بربط عام ومفاتيح مؤجَّلة، و`source_type` مفتاح أجنبي إلى سجل مغلق |
