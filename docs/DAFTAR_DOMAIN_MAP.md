...
- **السلطة الموقّعة للأوامر (P3-AL-55)**: `EXECUTE` على هذه الروتينات وصولٌ لا صلاحية. كل أمر حسّاس في المخزون والمشتريات والموردين يحمل توكيد `invctl/1` يصكّه الخادم بعد فحص الصلاحية والنطاق، بمفتاح مستقل عن مفتاحَي المحاسبة والتزويد، وتتحقق منه قاعدة البيانات مع بصمة الحمولة الفعلية قبل أي كتابة. الـGUC للعزل فقط، وليس دليل صلاحية.
