...
- **الربط المتعدد للصلاحية (المرحلة 3، P3-AL-15):** جدول ارتباط جديد
  ...
  **دورة الحياة، لا الزرع وحده (P3-AL-15 §A، تصحيح الجولة 3):** الزرع كلام عن يوم الترحيل وحده. **كل صف في `warehouses` يجب أن يقابله صف الفرع الأم `(business_id, branch_id, id)` في `branch_warehouses` ما دام الصف موجودًا، أيًّا كانت حالته `active` أو `archived`.** والسبب أن **كُتّاب المستودعات ثلاثة لا اثنان**: `StructureService.createBranch()` و`createWarehouse()`، والثالث `provision_create_business` داخل ترحيل **مجمَّد** (`0033_provisioner_atomic_authority.sql:160–164`) يُنشئ `'Main warehouse'` لكل نشاط جديد ولا تمرّ عليه أي شيفرة خدمة من المرحلة 3. لذلك الإنفاذ في القاعدة لا في الخدمة، وبأربعة كائنات: مُشغِّل `AFTER INSERT` عادي يكتب صف الفرع الأم لكل كاتب؛ ومُشغِّل قيد مؤجَّل `warehouses_require_home_branch` يرفض الإيداع إن غاب؛ و`branch_warehouses_keep_home` يرفض حذف صف الفرع الأم ما دام المستودع قائمًا؛ ورفض `BEFORE UPDATE` لأي تغيير في `warehouses.branch_id` (`inventory.warehouse_home_branch_immutable`)، لأن نقل الفرع الأم يعيد ترخيص تاريخ المستودع كلّه بصمت. **إضافة ارتباط غير الفرع الأم أو حذفه** يتطلب `warehouse.manage` **مع** `branch_scope_mode = 'all'` — لا يكفي أن يصل الفاعل إلى الطرفين، وإلا صارت العملية أداة توسيع صلاحية ذاتية.

  **الصلاحية الفعلية في القاعدة (P3-AL-54، تصحيح الجولة 4):** على الجدول `ENABLE` و`FORCE` لأمن الصفوف بطبقتَي سياسات `warehouses` نفسها (`0006_rls.sql:28–49`). `daftar_app` يملك `SELECT` فقط، ولا يملك `daftar_platform` ولا `daftar_provisioner` أي صلاحية عليه. المُشغِّل الحافظ ومُشغِّل الاكتمال ومُشغِّل الإبقاء على الفرع الأم كلها `SECURITY DEFINER` مملوكة لـ`daftar_inventory_internal` (دور `NOLOGIN` لا يدخل ولا عضوية لأي دور تشغيل فيه)، لأن الكاتب الثالث يعمل بصفة `daftar_platform`. والإضافة والحذف يمرّان بالروتينين `structure_associate_warehouse_branch` / `structure_dissociate_warehouse_branch` بعد فحص الصلاحية والنطاق في التطبيق، **ولا يكفي ذلك وحده (P3-AL-55، الجولة 5):** كل روتين يستهلك أولًا توكيد `invctl/1` موقّعًا يربط المستودع والفرع بعينهما، فاستدعاؤه مباشرةً ببيانات اعتماد `daftar_app` مع GUC مزيّفة يُرفض.
...
- **المرحلة 3 (P3-AL-05 §D) — قفل الوحدة بالتاريخ:** قبل وجود أي حركة مخزون لأي متغيّر من المنتج، يجوز تغيير `unit_code` و`unit_decimals` بالأمر المخوَّل — حتى لو كان التتبّع مفعَّلًا، لأن التفعيل تهيئة ولم يُقَس شيء بعد. **ومن أول حركة فصاعدًا يصيران غير قابلين للتغيير إلى الأبد.** بلوغ الرصيد صفرًا **لا** يفكّ القفل، وتعطيل التتبّع **لا** يفكّه، وإعادة التفعيل تستعمل الوحدة التاريخية نفسها بلا سؤال. السبب أن `stock_movements` تخزّن `qty_delta` ولا تلتقط الوحدة عمدًا (P3-AL-11)، فتغييرها يعيد تفسير كل كمية تاريخية بلا تعديل صفٍّ واحد ولا أثر. الرفض الثابت: `inventory.unit_identity_locked`، بمُشغِّل `BEFORE UPDATE ON products` تُنصِّبه P3-S2 (لأن جسم المُشغِّل لا يشير إلى جدول قبل وجوده) وقبل أول مُنتِج حركة حقيقي في P3-S3. وتحويل منتج تاريخي من `piece` إلى `kg` ليس تحويل وحدات بل **ممنوع**.
- **المرحلة 3 (P3-AL-52) — المتغيّر الأساس المخفي:** `product_variants.is_base` يُنشئه أمر تفعيل التتبّع وحده، و**كل قراءة كتالوج يراها التاجر تستثني `is_base = true`** — بما فيها قائمة المتغيّرات في `CatalogService.getProduct()` التي تُرجع اليوم كل صف غير مؤرشف. و`CHECK` يُبقي `sku` و`barcode` و`price_minor` فارغة و`attributes` `'{}'`، ومُشغِّل `product_variants_10_base_variant_authority` بحقوق المُستدعي يرفض أي إدراج أو تعديل على صفٍّ `is_base` إلا بصفة `daftar_inventory_internal` (`catalog.base_variant_not_mutable`) — على نمط `daftar_accounting_internal` المقبول في المرحلة 2. والحذف مغلق بالصلاحية: لا دور تشغيل يملك `DELETE` على `product_variants`، والدور الداخلي لا يملك `UPDATE` ولا `DELETE` عليه، ولا يُدرج إلا صفوفًا `is_base = true` (P3-AL-54 §F).
- **المرحلة 3 (P3-AL-04/P3-AL-05):** `products.unit` الحرّ **ليس** مرجعًا دلاليًا للكمية ولا يُفسَّر ولا يُستنتج منه شيء. تُضاف `products.track_inventory` (افتراضيًا `false` لكل منتج قائم)، و`unit_code`، و`unit_decimals`، مع `CHECK (track_inventory = false OR (unit_code IS NOT NULL AND unit_decimals IS NOT NULL))`. سجل الوحدات `units(unit_code, default_decimals, sort_order)` قابل للتوسّع بترحيل، وأسماؤه المترجمة في `unit_names(unit_code, locale, display_name)` خارج حقيقة المخزون. `unit_decimals` يُجمَّد على المنتج عند الاختيار، وتغيير `units.default_decimals` بترحيل لاحق لا يمسّ أي منتج قائم. **لا تحويل وحدات في المرحلة 3** — لا عمود معامل تحويل ولا وحدة أساس. **كاتب الأعمدة الثلاثة واحد (P3-AL-54):** الروتين `inventory_configure_product` المملوك لـ`daftar_inventory_internal`، ولا يُستدعى إلا بعد فحص `inventory.adjust`، ولا ينفّذ إلا بعد أن يستهلك توكيد `invctl/1` من نوع `inventory.configure_product` تُعاد حوسبة بصمة حمولته من وسائطه نفسها (P3-AL-55). `daftar_app` يحتفظ بـ`UPDATE` على مستوى الجدول لأعمدة الكتالوج (السعر، الفئة، SKU، الباركود، `products.unit`)، لكن المُشغِّل `products_10_inventory_config_authority` بحقوق المُستدعي يرفض أي إدراج أو تعديل مباشر لـ`track_inventory` أو `unit_code` أو `unit_decimals` برمز `inventory.configuration_authority_required`، ويليه بالترتيب `products_20_unit_history_lock` من P3-S2.
...
- **تمثيل التكلفة (قرار ملزم):** كل حقول **تكلفة الوحدة والمتوسط** الداخلية **`NUMERIC(28,10)`** — ممنوع Float/Double مطلقًا (Master §30)، وممنوع تمثيل تكلفة الوحدة بـBIGINT minor لأن التقريب المبكر يُفسد المتوسط المرجّح على الكميات الكبيرة. أمّا **قيمة الحركة** فهي BIGINT بالوحدة الصغرى عمدًا (P3-AL-49)، لأنها الرقم الذي يذهب إلى القيد بعينه:
  - `stock_levels.valuation_base_minor BIGINT` — **Cache** لمجموع قيم الحركات بالوحدة الصغرى الصحيحة، وليس مصدر حقيقة ثانيًا (المرحلة 3، P3-AL-01/P3-AL-49)
...
- **المرحلة 3 — قيد المخزون استثناء لا تحويل فيه:** مبلغ سطر المخزون هو `value_delta_base_minor` المخزَّن نفسه (BIGINT)، فلا تحويل ولا فرق تقريب ولا سطر 6100 على قيد مخزون (P3-AL-49). ما يلي هو عقد المرحلة 2 المقبول لبقية القيود، ولم يُمسَس:
...
- **المرحلة 3 (P3-AL-11):** لكل حركة مكوّنان صريحان — `qty_delta NUMERIC(18,4)` و**`value_delta_base_minor BIGINT`** (المرجع المالي، بالوحدة الصغرى الصحيحة) — مع `unit_cost_base_minor NUMERIC(28,10)` **لقطة للأثر** يكون `NULL` في حركة القيمة الصرفة. `CHECK (NOT (qty_delta = 0 AND value_delta_base_minor = 0))` و`CHECK ((qty_delta = 0) = (unit_cost_base_minor IS NULL))`. بذلك تُعاد الكمية والتقييم كلاهما من الحركات وحدها: `on_hand = Σ qty_delta` و`valuation_base_minor = Σ value_delta_base_minor` بترتيب `stock_seq` — **جمعُ قيمٍ مخزَّنة، بلا ضرب وبلا قسمة وبلا تقريب**، وهذا ما يجعل إعادة البناء دقيقة.
- **المرحلة 3 (P3-AL-49) — قانون دقة التقييم:**
  - **حدّ التقريب الوحيد في النظام كلّه، عند الحركة (تصحيح الجولة 2):** قيمة الحركة **عدد صحيح من الوحدات الصغرى**، وسطر القيد المحاسبي **هو نفس ذلك العدد** — فلا تحويل ثانٍ ولا تقريب ثانٍ. السبب: **التقريب ليس تجميعيًا** — عمليتان قيمة كل منهما `0.6` وحدة صغرى تُعطيان `1 + 1 = 2` للدفتر العام، بينما تقريب مجموعهما `round(1.2) = 1` — مقيسًا على PostgreSQL 16. بعد الكتابة تصبح **القيمة المخزَّنة** هي المرجع إلى الأبد، ولا يُعاد حسابها من `qty × avg` في أي تقرير ولا إعادة بناء. المعادلة والمتجهات التسعة في P3-AL-49.
  - `unit_cost_base_minor` **لقطة للأثر والتقارير**؛ `value_delta_base_minor` هو **مبلغ التقييم المرجعي**. ليسا مترادفين.
  - **ممنوع اشتقاق التقييم من `on_hand × avg`** في أي مسار (أمرًا كان أم إعادة بناء أم تقريرًا أم مطابقة). **ومسموح** للمتوسط أن يكون مُدخل التكلفة لحركة خارجة لاحقة تحت القفل — وهذا هو معنى سياسة المتوسط المرجّح أصلًا. مقيسًا: `on_hand = 3` و`valuation = 1` يعطي متوسطًا `0.3333333333` وحاصل ضرب `0.9999999999`.
...
- **المرحلة 3 (P3-AL-50/P3-AL-51) — هوية المصدر واكتماله فيزيائيًا:** الهوية الخماسية تمنع التكرار **فقط**، ولا تثبت أن المصدر حقيقي. لذلك: `source_type` مفتاح أجنبي إلى سجل مغلق `stock_source_types`؛ وجدول `stock_source_bindings` **مفتاحه خماسي** `(business_id, source_type, source_id, source_line_id, movement_kind)` — وليس رباعيًا كما كان مكتوبًا، لأن سطر التحويل الواحد يولّد **حركتين**، فالرباعي ليس فريدًا ولا يمكن أن يكون هدفًا لـFK. بالمفتاح الخماسي تصبح المفاتيح الأجنبية **المؤجّلة في الاتجاهين** ممكنة فعلاً. ولإثبات أن المصدر موجود حقًّا يُنشئ كلّ نوع مصدر **جسرًا خاصًا به** يحمل FK حقيقيًا إلى سطر النطاق (ومعه `ON DELETE RESTRICT` الذي لم يكن ممكنًا على الجدول العام) وFK إلى الربط، مع Trigger مؤجّل **على جدول الربط نفسه** يطلب صف الجسر — لأن Trigger على جدول المصدر وحده **لا يُطلَق أصلًا حين لا يوجد صف مصدر**. ولا يزال `stock_movements` بلا أي FK متعدد الأشكال.
