...
### 7.1 سجل الأسعار — منفَّذ في P2-S5 (مرشَّح)

القاعدة رقم 2 أعلاه صارت مخطَّطًا لا خطةً، مع ثلاثة فوارق مقصودة عن الصياغة المجرّدة:

- **`accounting_fx_rates`** سجلّ لا يُعدَّل ولا يُحذف، صف واحد لكل (منشأة، زوج موجَّه، لحظة). التصحيح = صف جديد بلحظة لاحقة، وليس تعديل القديم. مشغّل `BEFORE UPDATE OR DELETE` يرفض بلا استثناء هوية — حتى مالك المخطَّط.
- **لا مزوّد اليوم ولا مكان له في المخطَّط**: `source` مثبَّت بـ`CHECK` على `'manual'` وحدها. قيمة `provider` لا يستطيع أي مسار كتابتها وعد يقطعه المخطَّط ولا يستطيع الكود الوفاء به. OD-11 يبقى مفتوحًا ويصل بميجريشن خاصة به ومراجعة خاصة به.
- **البحث حتمي**: آخر سعر أعلنته المنشأة لهذا الزوج بالذات عند اللحظة أو قبلها، وإلا فرفض باسمه `accounting.fx_rate_missing`. **لا مقلوب، ولا سعر مركّب عبر عملة ثالثة، ولا 1.0 ضمنية لزوج أجنبي، ولا سعر مستقبلي.**

وفروقات القاعدة رقم 4 (4900/6900 مقابل 6100 مقابل 6200) صارت خاصية نوعية لا مراجعة: `classifyRealizedFx` لا تستطيع إرجاع غير `fx_gain`/دائن أو `fx_loss`/مدين، و`classifyRoundingResidual` لا تستطيع إرجاع غير `rounding`، ولا واحدة منهما تصل إلى `purchase_price_variance`. الإثبات شامل في `packages/accounting/test/realized-fx.test.ts`. الدالتان **تقرران ولا تُرحّلان**.

الحالة: مرشَّح قيد المراجعة — `docs/PHASE_2_S5_ACCEPTANCE.md`.
