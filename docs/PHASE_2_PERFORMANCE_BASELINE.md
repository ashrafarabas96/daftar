...
- **Every table a measured read touches is ANALYZEd before the measurement**, not only the two the generator fills. This rule is written down because it was learned from a failure: `accounts` is read by the trial balance, written by nothing in the dataset generator, and about twenty rows per business — under `autovacuum_analyze_threshold`, so nothing analyzed it by itself. It entered the measurement with `relpages = 0`, `reltuples = -1` and `last_analyze` null. The planner estimated **one** row where there were twenty-one, and one estimated row on the outer side of a join makes re-executing the inner side look free: it chose a nested loop over the whole journal aggregate and re-ran it **once per account**. That cost 327 ms on this workstation against 125 ms with statistics, and **2.9 s on a GitHub runner**, where the re-executed side is a parallel `Gather Merge` that cannot be reused between loops. A production `accounts` is one multi-tenant table far past the autovacuum threshold and never looks like this, so the budget was measuring the absence of statistics rather than the cost of the policy family. `analyzeMeasuredTables` now establishes them deterministically, every evidence file records `planningStatistics` (when each measured table was last analyzed), the budget suite fails if any of them is null, and `tests/security/phase2-s8-gate-tamper.test.ts` proves the release gate refuses a file measured in the dark.
