...
| 18 | Exactly one **CURRENT** POSTED opening balance per business **at any instant** — a bound on what stands, never on how many sets a business has posted over its life (`draft → posted → reversal → superseded`, then a replacement may be posted) | partial unique index over `status = 'posted'` | "refuses a second posted set…"; "at most one posted set survives…"; "replaces a posted set the only legal way: reverse, supersede, post again (§32)" | **ENFORCED** |
