...
**7.2 (WITHDRAWN in P2-S9) `daftar_migrator` cannot apply the accepted migration history at all.** This finding's observation was right and its explanation was wrong, and the explanation is withdrawn. P2-S9 answered §36 by performing the deployment instead of reasoning about it: `npm run check:deployment-authority` applies `0000` through `0052` as `daftar_migrator` and nothing else, from an empty database, and carries a database at `0039` and one at `0050` forward to `0052` the same way. The cause was never the history: `bootstrap.sql` gave the deployer one of the two memberships the history needs, and schema `public` belonged to `pg_database_owner`, so the deployer held `CREATE` without grant option. Both are corrected in `bootstrap.sql` and in the runner, no frozen migration was touched and no runtime principal was widened, and RB-P2-01 is closed — see `PHASE_2_S9_RELEASE.md` §3. What was recorded at the time, kept as history:
...
At the time each was answered the same way, and one half of that answer stands: **the migration principal is never widened to make a migration apply**, and P2-S9's correction widened nothing. The other half — that this is a property of the accepted history, and that DAFTAR's deployment principal must therefore be the administrator — is **refuted**. The third bullet above is the one that survives, and only for a restored copy: objects come out owned by whoever performed the restore, which is a recovery-procedure step rather than a property of the history.
...
| ~~**§36**~~ | ~~`daftar_migrator` cannot apply the accepted migration history~~ | **WITHDRAWN and closed in P2-S9** as release blocker RB-P2-01. It is applied as `daftar_migrator`, from empty and across both upgrade paths, with no frozen migration touched and no principal widened; the corrections are in `bootstrap.sql` and the runner. See §7.2 and `PHASE_2_S9_RELEASE.md` §3 |
