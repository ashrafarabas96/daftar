...
export function Spinner({ label }: { label?: string }) {
  return (
    <div role="status" aria-label={label ?? 'loading'} style={{ display: 'inline-flex', alignItems: 'center', gap: spacing[2], fontFamily: typography.fontFamily.base }}>
      <span
        style={{
          width: '1.25rem', height: '1.25rem', borderRadius: radius.full,
          border: `2px solid ${colors.neutral[200]}`, borderTopColor: colors.brand.primary,
          animation: 'daftar-spin 0.8s linear infinite', display: 'inline-block',
        }}
      />
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundSize: '200% 100%', animation: 'daftar-shimmer 1.4s infinite', ...style,
      }}
    />
  );
}
...
function StateShell(props: { icon: string; title: string; description?: string; action?: ReactNode }) {
  return (
    <div style={{ textAlign: 'center', padding: `${spacing[10]} ${spacing[4]}`, fontFamily: typography.fontFamily.base }}>
      <div aria-hidden style={{ fontSize: '2.5rem', marginBottom: spacing[3] }}>{props.icon}</div>
...
export function ErrorState(props: { title: string; description?: string; retryLabel?: string; onRetry?: () => void }) {
  return <StateShell icon="⚠" title={props.title} description={props.description}
    action={props.retryLabel ? <Button variant="secondary" onClick={props.onRetry}>{props.retryLabel}</Button> : undefined} />;
}
...
export function PlanLimitState(props: { title: string; description?: string; upgradeLabel?: string; onUpgrade?: () => void }) {
  return <StateShell icon="▲" title={props.title} description={props.description}
    action={props.upgradeLabel ? <Button variant="secondary" onClick={props.onUpgrade}>{props.upgradeLabel}</Button> : undefined} />;
}
